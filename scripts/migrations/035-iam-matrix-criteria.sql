...
VALUES
  ('iam', 'auth_protocols_standards', 'Authentication Protocols & Standards', 'Authentifizierungsprotokolle & Standards', 'Supported authentication protocols, identity standards, and directory integrations.', 'Unterstützte Authentifizierungsprotokolle, Identitätsstandards und Verzeichnisintegrationen.', 100),
  ('iam', 'mfa_verification', 'Multi-Factor Authentication', 'Multi-Faktor-Authentifizierung', 'Multi-factor authentication capabilities, methods, and enforcement options.', 'Multi-Faktor-Authentifizierungsfähigkeiten, Methoden und Durchsetzungsoptionen.', 200),
  ('iam', 'user_management', 'User Management', 'Benutzerverwaltung', 'User registration, lifecycle management, federation, and account recovery.', 'Benutzerregistrierung, Lebenszyklusverwaltung, Föderation und Kontowiederherstellung.', 300),
  ...
  ('iam', 'access_control', 'Access Control', 'Zugriffskontrolle', 'Access control models, permissions, multi-tenancy, and delegation.', 'Zugriffskontrollmodelle, Berechtigungen, Mandantenfähigkeit und Delegation.', 500),
  ('iam', 'deployment_operations', 'Deployment & Operations', 'Bereitstellung & Betrieb', 'Deployment models, container support, databases, and scaling capabilities.', 'Bereitstellungsmodelle, Container-Unterstützung, Datenbanken und Skalierungsfähigkeiten.', 600),
...
FROM (
  SELECT 'iam' AS category_id, 'auth_protocols_standards' AS group_key, 'oidc_support' AS criterion_key, 'OpenID Connect support' AS label_en, 'OpenID-Connect-Unterstützung' AS label_de, 'boolean' AS value_type, 'beneficial' AS semantics, 'must_match' AS filter_mode, 1010 AS sort_order, 'Whether the product supports OpenID Connect for authentication and identity federation.' AS help_text_en, 'Ob das Produkt OpenID Connect für Authentifizierung und Identitätsföderation unterstützt.' AS help_text_de
  UNION ALL SELECT 'iam', 'auth_protocols_standards', 'saml_support', 'SAML 2.0 support', 'SAML-2.0-Unterstützung', 'boolean', 'beneficial', 'must_match', 1020, 'Whether the product supports SAML 2.0 for enterprise single sign-on.', 'Ob das Produkt SAML 2.0 für Enterprise-Single-Sign-On unterstützt.'
  UNION ALL SELECT 'iam', 'auth_protocols_standards', 'oauth2_support', 'OAuth 2.0 support', 'OAuth-2.0-Unterstützung', 'boolean', 'beneficial', 'optional', 1030, 'Whether the product supports OAuth 2.0 for authorization flows.', 'Ob das Produkt OAuth 2.0 für Autorisierungsabläufe unterstützt.'
  ...
  UNION ALL SELECT 'iam', 'auth_protocols_standards', 'scim_support', 'SCIM provisioning support', 'SCIM-Bereitstellungsunterstützung', 'boolean', 'beneficial', 'optional', 1050, 'Whether the product supports SCIM for automated user provisioning and deprovisioning.', 'Ob das Produkt SCIM für automatisierte Benutzerbereitstellung und -deprovisionierung unterstützt.'
  UNION ALL SELECT 'iam', 'auth_protocols_standards', 'radius_support', 'RADIUS support', 'RADIUS-Unterstützung', 'boolean', 'informational', 'optional', 1060, 'Whether the product supports RADIUS for network access authentication.', 'Ob das Produkt RADIUS für Netzwerkzugangsauthentifizierung unterstützt.'
  UNION ALL SELECT 'iam', 'auth_protocols_standards', 'passkey_webauthn', 'Passkey / WebAuthn support', 'Passkey-/WebAuthn-Unterstützung', 'boolean', 'beneficial', 'optional', 1070, 'Whether the product supports passkeys and WebAuthn for passwordless authentication.', 'Ob das Produkt Passkeys und WebAuthn für passwortlose Authentifizierung unterstützt.'
  UNION ALL SELECT 'iam', 'mfa_verification', 'builtin_mfa', 'Built-in MFA support', 'Integrierte MFA-Unterstützung', 'boolean', 'beneficial', 'must_match', 2010, 'Whether the product includes built-in multi-factor authentication.', 'Ob das Produkt integrierte Multi-Faktor-Authentifizierung enthält.'
  UNION ALL SELECT 'iam', 'mfa_verification', 'mfa_methods', 'Supported MFA methods', 'Unterstützte MFA-Methoden', 'multi_enum', 'informational', 'multi_select', 2020, 'Which second-factor methods are available for user authentication.', 'Welche Zweitfaktor-Methoden für die Benutzerauthentifizierung verfügbar sind.'
  UNION ALL SELECT 'iam', 'mfa_verification', 'adaptive_mfa', 'Adaptive / risk-based MFA', 'Adaptive / risikobasierte MFA', 'boolean', 'beneficial', 'optional', 2030, 'Whether MFA challenges adapt based on risk signals such as location or device.', 'Ob MFA-Abfragen basierend auf Risikosignalen wie Standort oder Gerät angepasst werden.'
  UNION ALL SELECT 'iam', 'mfa_verification', 'mfa_enforcement_granularity', 'MFA enforcement granularity', 'MFA-Durchsetzungsgranularität', 'enum', 'informational', 'optional', 2040, 'At which level MFA requirements can be enforced across the organization.', 'Auf welcher Ebene MFA-Anforderungen in der Organisation durchgesetzt werden können.'
  UNION ALL SELECT 'iam', 'user_management', 'self_service_registration', 'Self-service registration', 'Self-Service-Registrierung', 'boolean', 'informational', 'optional', 3010, 'Whether end users can register accounts without administrator intervention.', 'Ob Endbenutzer Konten ohne Administratoreingriff registrieren können.'
  ...
  UNION ALL SELECT 'iam', 'user_management', 'user_lifecycle_management', 'User lifecycle management', 'Benutzerlebenszyklus-Verwaltung', 'multi_enum', 'informational', 'multi_select', 3030, 'Which user lifecycle stages are supported from onboarding to offboarding.', 'Welche Benutzerlebenszyklusphasen von der Einrichtung bis zur Deaktivierung unterstützt werden.'
  UNION ALL SELECT 'iam', 'user_management', 'user_federation_sources', 'User federation sources', 'Benutzerföderationsquellen', 'multi_enum', 'informational', 'multi_select', 3040, 'External identity sources that can be federated into the user directory.', 'Externe Identitätsquellen, die in das Benutzerverzeichnis föderiert werden können.'
  UNION ALL SELECT 'iam', 'user_management', 'account_recovery_methods', 'Account recovery methods', 'Kontowiederherstellungsmethoden', 'multi_enum', 'informational', 'multi_select', 3050, 'Available methods for users to recover access to their accounts.', 'Verfügbare Methoden für Benutzer zur Wiederherstellung des Kontozugangs.'
  UNION ALL SELECT 'iam', 'user_management', 'user_profile_customization', 'Custom user attributes', 'Benutzerdefinierte Benutzerattribute', 'boolean', 'informational', 'optional', 3060, 'Whether custom attributes can be added to user profiles beyond standard fields.', 'Ob benutzerdefinierte Attribute über Standardfelder hinaus zu Benutzerprofilen hinzugefügt werden können.'
  UNION ALL SELECT 'iam', 'sso_federation', 'sso_model', 'SSO implementation model', 'SSO-Implementierungsmodell', 'enum', 'informational', 'optional', 4010, 'Which single sign-on initiation flows the product supports.', 'Welche Single-Sign-On-Initiierungsabläufe das Produkt unterstützt.'
  ...
  UNION ALL SELECT 'iam', 'sso_federation', 'social_login_providers', 'Social login providers', 'Social-Login-Anbieter', 'multi_enum', 'informational', 'multi_select', 4030, 'Which social identity providers are available for user login.', 'Welche sozialen Identitätsanbieter für die Benutzeranmeldung verfügbar sind.'
  UNION ALL SELECT 'iam', 'sso_federation', 'app_catalog_size', 'Pre-built app integrations', 'Vorgefertigte App-Integrationen', 'enum', 'informational', 'optional', 4040, 'Number of pre-built application integrations available in the catalog.', 'Anzahl der im Katalog verfügbaren vorgefertigten Anwendungsintegrationen.'
  UNION ALL SELECT 'iam', 'sso_federation', 'session_management', 'Session management features', 'Sitzungsverwaltungsfunktionen', 'multi_enum', 'informational', 'multi_select', 4050, 'Available session management capabilities for controlling active user sessions.', 'Verfügbare Sitzungsverwaltungsfähigkeiten zur Kontrolle aktiver Benutzersitzungen.'
  UNION ALL SELECT 'iam', 'access_control', 'access_control_model', 'Access control model', 'Zugriffskontrollmodell', 'multi_enum', 'informational', 'multi_select', 5010, 'Which access control paradigms the product implements for authorization decisions.', 'Welche Zugriffskontrollparadigmen das Produkt für Autorisierungsentscheidungen implementiert.'
  UNION ALL SELECT 'iam', 'access_control', 'fine_grained_permissions', 'Fine-grained permissions', 'Feingranulare Berechtigungen', 'boolean', 'informational', 'optional', 5020, 'Whether permissions can be assigned at a resource or action level beyond roles.', 'Ob Berechtigungen auf Ressourcen- oder Aktionsebene über Rollen hinaus vergeben werden können.'
  UNION ALL SELECT 'iam', 'access_control', 'multi_tenancy', 'Multi-tenancy support', 'Mandantenfähigkeit', 'boolean', 'beneficial', 'optional', 5030, 'Whether the product supports multiple isolated tenants within a single deployment.', 'Ob das Produkt mehrere isolierte Mandanten innerhalb einer einzelnen Installation unterstützt.'
  UNION ALL SELECT 'iam', 'access_control', 'consent_management', 'Consent management', 'Einwilligungsverwaltung', 'boolean', 'informational', 'optional', 5040, 'Whether the product includes user consent collection and management features.', 'Ob das Produkt Funktionen zur Erfassung und Verwaltung von Benutzereinwilligungen enthält.'
  UNION ALL SELECT 'iam', 'access_control', 'delegated_administration', 'Delegated administration', 'Delegierte Verwaltung', 'boolean', 'informational', 'optional', 5050, 'Whether administrative tasks can be delegated to non-global administrators.', 'Ob Verwaltungsaufgaben an nicht-globale Administratoren delegiert werden können.'
  UNION ALL SELECT 'iam', 'deployment_operations', 'deployment_model', 'Deployment model', 'Bereitstellungsmodell', 'multi_enum', 'tradeoff', 'multi_select', 6010, 'Available deployment options ranging from self-hosted to fully managed cloud.', 'Verfügbare Bereitstellungsoptionen von selbst gehostet bis vollständig verwalteter Cloud.'
  UNION ALL SELECT 'iam', 'deployment_operations', 'container_support', 'Container support', 'Container-Unterstützung', 'multi_enum', 'informational', 'multi_select', 6020, 'Supported container and orchestration formats for deployment.', 'Unterstützte Container- und Orchestrierungsformate für die Bereitstellung.'
  UNION ALL SELECT 'iam', 'deployment_operations', 'high_availability', 'High availability support', 'Hochverfügbarkeitsunterstützung', 'boolean', 'beneficial', 'optional', 6030, 'Whether the product supports high availability configurations with failover.', 'Ob das Produkt Hochverfügbarkeitskonfigurationen mit Failover unterstützt.'
  UNION ALL SELECT 'iam', 'deployment_operations', 'database_backends', 'Supported database backends', 'Unterstützte Datenbank-Backends', 'multi_enum', 'informational', 'multi_select', 6040, 'Which database systems can be used as the storage backend.', 'Welche Datenbanksysteme als Speicher-Backend verwendet werden können.'
  UNION ALL SELECT 'iam', 'deployment_operations', 'data_processing_location', 'Data processing location', 'Datenverarbeitungsstandort', 'enum', 'tradeoff', 'must_match', 6050, 'Where identity and authentication data is stored and processed.', 'Wo Identitäts- und Authentifizierungsdaten gespeichert und verarbeitet werden.'
  UNION ALL SELECT 'iam', 'deployment_operations', 'horizontal_scaling', 'Horizontal scaling', 'Horizontale Skalierung', 'boolean', 'beneficial', 'optional', 6060, 'Whether the product supports horizontal scaling across multiple nodes.', 'Ob das Produkt horizontale Skalierung über mehrere Knoten unterstützt.'
  UNION ALL SELECT 'iam', 'customization_developer', 'login_page_theming', 'Login page theming', 'Login-Seiten-Theming', 'enum', 'informational', 'optional', 7010, 'Level of visual customization available for login and registration pages.', 'Grad der visuellen Anpassung für Login- und Registrierungsseiten.'
  UNION ALL SELECT 'iam', 'customization_developer', 'auth_flow_customization', 'Authentication flow customization', 'Authentifizierungsfluss-Anpassung', 'enum', 'informational', 'optional', 7020, 'How authentication and registration flows can be customized.', 'Wie Authentifizierungs- und Registrierungsabläufe angepasst werden können.'
  UNION ALL SELECT 'iam', 'customization_developer', 'admin_api_available', 'Admin API available', 'Admin-API verfügbar', 'boolean', 'beneficial', 'optional', 7030, 'Whether a programmatic administration API is available for automation.', 'Ob eine programmierbare Verwaltungs-API für Automatisierung verfügbar ist.'
  UNION ALL SELECT 'iam', 'customization_developer', 'sdk_availability', 'SDK availability', 'SDK-Verfügbarkeit', 'multi_enum', 'informational', 'multi_select', 7040, 'Programming languages and platforms for which official SDKs are provided.', 'Programmiersprachen und Plattformen, für die offizielle SDKs bereitgestellt werden.'
  UNION ALL SELECT 'iam', 'customization_developer', 'webhook_event_support', 'Webhook / event support', 'Webhook-/Ereignisunterstützung', 'boolean', 'beneficial', 'optional', 7050, 'Whether the product supports webhooks or event streams for real-time integrations.', 'Ob das Produkt Webhooks oder Ereignisstroms für Echtzeit-Integrationen unterstützt.'
  UNION ALL SELECT 'iam', 'customization_developer', 'documentation_quality', 'Documentation quality', 'Dokumentationsqualität', 'enum', 'informational', 'optional', 7060, 'Overall quality and completeness of the available developer documentation.', 'Gesamtqualität und Vollständigkeit der verfügbaren Entwicklerdokumentation.'
  UNION ALL SELECT 'iam', 'security_compliance', 'brute_force_protection', 'Brute-force protection', 'Brute-Force-Schutz', 'boolean', 'beneficial', 'optional', 8010, 'Whether the product includes built-in brute-force and credential-stuffing protection.', 'Ob das Produkt integrierten Brute-Force- und Credential-Stuffing-Schutz enthält.'
  UNION ALL SELECT 'iam', 'security_compliance', 'audit_logging', 'Audit logging', 'Audit-Protokollierung', 'enum', 'beneficial', 'optional', 8020, 'Depth and scope of security event logging for compliance and forensics.', 'Tiefe und Umfang der Sicherheitsereignisprotokollierung für Compliance und Forensik.'
  UNION ALL SELECT 'iam', 'security_compliance', 'security_audit_completed', 'Independent security audit', 'Unabhängiges Sicherheitsaudit', 'boolean', 'beneficial', 'optional', 8030, 'Whether the product has undergone an independent third-party security audit.', 'Ob das Produkt ein unabhängiges Sicherheitsaudit durch Dritte durchlaufen hat.'
  UNION ALL SELECT 'iam', 'security_compliance', 'gdpr_dpa_available', 'GDPR DPA available', 'DSGVO-AVV verfügbar', 'boolean', 'informational', 'optional', 8040, 'Whether a GDPR-compliant data processing agreement is available for customers.', 'Ob ein DSGVO-konformer Auftragsverarbeitungsvertrag für Kunden verfügbar ist.'
  UNION ALL SELECT 'iam', 'security_compliance', 'data_export_available', 'Data export available', 'Datenexport verfügbar', 'boolean', 'beneficial', 'optional', 8050, 'Whether user and configuration data can be exported in a portable format.', 'Ob Benutzer- und Konfigurationsdaten in einem portablen Format exportiert werden können.'
  UNION ALL SELECT 'iam', 'security_compliance', 'account_deletion', 'Account deletion available', 'Kontolöschung verfügbar', 'boolean', 'beneficial', 'optional', 8060, 'Whether end users or administrators can fully delete user accounts and data.', 'Ob Endbenutzer oder Administratoren Benutzerkonten und Daten vollständig löschen können.'
  ...
  UNION ALL SELECT 'iam', 'pricing_fit', 'free_tier_available', 'Free tier available', 'Kostenlose Stufe verfügbar', 'boolean', 'informational', 'must_match', 9020, 'Whether a free tier is available without requiring payment.', 'Ob eine kostenlose Stufe ohne Bezahlung verfügbar ist.'
  ...
  UNION ALL SELECT 'iam', 'pricing_fit', 'enterprise_plan', 'Enterprise plan available', 'Enterprise-Tarif verfügbar', 'boolean', 'informational', 'optional', 9040, 'Whether a dedicated enterprise plan with premium support is available.', 'Ob ein dedizierter Enterprise-Tarif mit Premium-Support verfügbar ist.'
  UNION ALL SELECT 'iam', 'pricing_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9050, 'Target audience profiles the product is best suited for.', 'Zielgruppenprofile, für die das Produkt am besten geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'iam', 'password_policy_engine', 'basic', 'Basic (length / complexity)', 'Basis (Länge / Komplexität)', 'neutral', 10
  ...
  UNION ALL SELECT 'iam', 'user_lifecycle_management', 'account_deletion', 'Account deletion', 'Kontolöschung', 'neutral', 60
  ...
  UNION ALL SELECT 'iam', 'account_recovery_methods', 'admin_reset', 'Admin reset', 'Admin-Zurücksetzung', 'neutral', 40
  ...
  UNION ALL SELECT 'iam', 'session_management', 'idle_timeout', 'Idle timeout', 'Inaktivitäts-Timeout', 'neutral', 40
  ...
  UNION ALL SELECT 'iam', 'data_processing_location', 'eu_primary', 'EU primary, non-EU fallback', 'EU primär, Nicht-EU-Ausweichstandort', 'neutral', 20
  ...
  UNION ALL SELECT 'iam', 'source_model', 'proprietary', 'Proprietary', 'Proprietär', 'warning', 40
  ...
  UNION ALL SELECT 'iam', 'free_tier_user_limit', 'over_10000', 'Over 10,000', 'Über 10.000', 'neutral', 20
  ...
  UNION ALL SELECT 'iam', 'fit_profiles', 'enterprise', 'Enterprise', 'Großunternehmen', 'neutral', 30
