...
VALUES
  ('image-generation', 'generation_capabilities', 'Generation Capabilities', 'Generierungsfaehigkeiten', 'Core image generation abilities including resolution, styles, and modalities.', 'Kernfaehigkeiten der Bildgenerierung einschliesslich Aufloesung, Stile und Modalitaeten.', 100),
  ...
  ('image-generation', 'integration_access', 'Integration & Access', 'Integration & Zugang', 'API access, supported platforms, plugin ecosystem, and creative tool integrations.', 'API-Zugang, unterstuetzte Plattformen, Plugin-Oekosystem und Kreativtool-Integrationen.', 600),
  ('image-generation', 'openness_audit', 'Openness & Audit', 'Offenheit & Audit', 'Model weights, source code availability, licensing, and security audits.', 'Modellgewichte, Quellcode-Verfuegbarkeit, Lizenzierung und Sicherheitsaudits.', 700),
...
FROM (
  SELECT 'image-generation' AS category_id, 'generation_capabilities' AS group_key, 'max_output_resolution' AS criterion_key, 'Maximum output resolution' AS label_en, 'Maximale Ausgabeaufloesung' AS label_de, 'enum' AS value_type, 'informational' AS semantics, 'optional' AS filter_mode, 1010 AS sort_order, 'The highest image resolution the tool can generate.' AS help_text_en, 'Die hoechste Bildaufloesung, die das Tool generieren kann.' AS help_text_de
  UNION ALL SELECT 'image-generation', 'generation_capabilities', 'supported_styles', 'Supported artistic styles', 'Unterstuetzte kuenstlerische Stile', 'multi_enum', 'informational', 'multi_select', 1020, 'Which artistic styles the tool supports for image generation.', 'Welche kuenstlerischen Stile das Tool fuer die Bildgenerierung unterstuetzt.'
  UNION ALL SELECT 'image-generation', 'generation_capabilities', 'image_to_image', 'Image-to-image generation', 'Bild-zu-Bild-Generierung', 'boolean', 'informational', 'optional', 1030, 'Whether existing images can be used as input for transformation.', 'Ob bestehende Bilder als Eingabe fuer Transformationen verwendet werden koennen.'
  UNION ALL SELECT 'image-generation', 'generation_capabilities', 'video_generation', 'Video generation', 'Videogenerierung', 'boolean', 'informational', 'optional', 1040, 'Whether the tool supports generating video content.', 'Ob das Tool die Generierung von Videoinhalten unterstuetzt.'
  UNION ALL SELECT 'image-generation', 'generation_capabilities', 'batch_generation', 'Batch generation', 'Stapelgenerierung', 'boolean', 'beneficial', 'optional', 1050, 'Whether multiple images can be generated in a single batch.', 'Ob mehrere Bilder in einem einzigen Stapel generiert werden koennen.'
  ...
  UNION ALL SELECT 'image-generation', 'model_architecture', 'model_type', 'Model architecture type', 'Modellarchitekturtyp', 'enum', 'informational', 'optional', 2010, 'The underlying model architecture used for image generation.', 'Die zugrundeliegende Modellarchitektur fuer die Bildgenerierung.'
  UNION ALL SELECT 'image-generation', 'model_architecture', 'fine_tuning_support', 'Fine-tuning support', 'Feinabstimmungs-Unterstuetzung', 'boolean', 'informational', 'optional', 2020, 'Whether the model can be fine-tuned on custom data.', 'Ob das Modell mit eigenen Daten feinabgestimmt werden kann.'
  UNION ALL SELECT 'image-generation', 'model_architecture', 'training_data_transparency', 'Training data transparency', 'Trainingsdaten-Transparenz', 'enum', 'tradeoff', 'optional', 2030, 'How transparent the provider is about training data sources.', 'Wie transparent der Anbieter bezueglich der Trainingsdatenquellen ist.'
  UNION ALL SELECT 'image-generation', 'model_architecture', 'custom_model_training', 'Custom model training', 'Benutzerdefiniertes Modelltraining', 'boolean', 'informational', 'optional', 2040, 'Whether users can train entirely custom models.', 'Ob Nutzer vollstaendig eigene Modelle trainieren koennen.'
  UNION ALL SELECT 'image-generation', 'input_control', 'prompt_type', 'Prompt input type', 'Prompt-Eingabetyp', 'multi_enum', 'informational', 'multi_select', 3010, 'Which input types are accepted for directing image generation.', 'Welche Eingabetypen fuer die Steuerung der Bildgenerierung akzeptiert werden.'
  UNION ALL SELECT 'image-generation', 'input_control', 'negative_prompts', 'Negative prompt support', 'Negativ-Prompt-Unterstuetzung', 'boolean', 'informational', 'optional', 3020, 'Whether negative prompts can be used to exclude unwanted elements.', 'Ob Negativ-Prompts verwendet werden koennen, um unerwuenschte Elemente auszuschliessen.'
  UNION ALL SELECT 'image-generation', 'input_control', 'inpainting', 'Inpainting support', 'Inpainting-Unterstuetzung', 'boolean', 'informational', 'optional', 3030, 'Whether specific regions of an image can be edited.', 'Ob bestimmte Bereiche eines Bildes bearbeitet werden koennen.'
  UNION ALL SELECT 'image-generation', 'input_control', 'outpainting', 'Outpainting support', 'Outpainting-Unterstuetzung', 'boolean', 'informational', 'optional', 3040, 'Whether images can be extended beyond their original borders.', 'Ob Bilder ueber ihre urspruenglichen Grenzen hinaus erweitert werden koennen.'
  UNION ALL SELECT 'image-generation', 'input_control', 'controlnet_support', 'ControlNet or guided generation', 'ControlNet oder gefuehrte Generierung', 'boolean', 'informational', 'optional', 3050, 'Whether advanced structural control like pose or depth guidance is supported.', 'Ob erweiterte strukturelle Steuerung wie Pose- oder Tiefenfuehrung unterstuetzt wird.'
  UNION ALL SELECT 'image-generation', 'input_control', 'upscaling', 'Image upscaling', 'Bildhochskalierung', 'boolean', 'beneficial', 'optional', 3060, 'Whether the tool can enhance image resolution.', 'Ob das Tool die Bildaufloesung verbessern kann.'
  UNION ALL SELECT 'image-generation', 'output_usage', 'output_formats', 'Output formats', 'Ausgabeformate', 'multi_enum', 'informational', 'multi_select', 4010, 'Which image file formats are supported for output.', 'Welche Bilddateiformate fuer die Ausgabe unterstuetzt werden.'
  UNION ALL SELECT 'image-generation', 'output_usage', 'commercial_use_allowed', 'Commercial use of output', 'Kommerzielle Nutzung der Ausgabe', 'enum', 'tradeoff', 'must_match', 4020, 'Whether generated images may be used commercially.', 'Ob generierte Bilder kommerziell genutzt werden duerfen.'
  ...
  UNION ALL SELECT 'image-generation', 'integration_access', 'api_access', 'API access', 'API-Zugang', 'boolean', 'beneficial', 'optional', 6010, 'Whether programmatic API access is available.', 'Ob programmatischer API-Zugang verfuegbar ist.'
  UNION ALL SELECT 'image-generation', 'integration_access', 'supported_platforms', 'Supported platforms', 'Unterstuetzte Plattformen', 'multi_enum', 'informational', 'multi_select', 6020, 'Which platforms and operating systems are supported.', 'Welche Plattformen und Betriebssysteme unterstuetzt werden.'
  UNION ALL SELECT 'image-generation', 'integration_access', 'plugin_ecosystem', 'Plugin or extension ecosystem', 'Plugin- oder Erweiterungs-Oekosystem', 'boolean', 'informational', 'optional', 6030, 'Whether a plugin or extension ecosystem is available.', 'Ob ein Plugin- oder Erweiterungs-Oekosystem verfuegbar ist.'
  ...
  UNION ALL SELECT 'image-generation', 'openness_audit', 'model_weights_availability', 'Model weights availability', 'Verfuegbarkeit der Modellgewichte', 'enum', 'tradeoff', 'optional', 7010, 'Whether model weights are publicly available.', 'Ob Modellgewichte oeffentlich verfuegbar sind.'
  UNION ALL SELECT 'image-generation', 'openness_audit', 'source_code_model', 'Source code model', 'Quellcode-Modell', 'enum', 'tradeoff', 'optional', 7020, 'Whether the platform source code is open source, source available, or proprietary.', 'Ob der Plattform-Quellcode quelloffen, quelleinsehbar oder proprietaer ist.'
  ...
  UNION ALL SELECT 'image-generation', 'openness_audit', 'security_audit', 'Independent security audit', 'Unabhaengiges Sicherheitsaudit', 'boolean', 'beneficial', 'optional', 7040, 'Whether a third-party security audit has been performed.', 'Ob ein Sicherheitsaudit durch Dritte durchgefuehrt wurde.'
  UNION ALL SELECT 'image-generation', 'openness_audit', 'model_card_published', 'Model card published', 'Modellkarte veroeffentlicht', 'boolean', 'informational', 'optional', 7050, 'Whether a model card documenting capabilities and limitations is published.', 'Ob eine Modellkarte mit Faehigkeiten und Einschraenkungen veroeffentlicht ist.'
  ...
  UNION ALL SELECT 'image-generation', 'pricing_fit', 'free_tier_available', 'Free tier available', 'Kostenlose Stufe verfuegbar', 'boolean', 'informational', 'optional', 8020, 'Whether a free tier is available without payment.', 'Ob eine kostenlose Stufe ohne Bezahlung verfuegbar ist.'
  UNION ALL SELECT 'image-generation', 'pricing_fit', 'credit_system', 'Credit or token system', 'Credit- oder Token-System', 'boolean', 'informational', 'optional', 8030, 'Whether usage is metered via a credit or token system.', 'Ob die Nutzung ueber ein Credit- oder Token-System gemessen wird.'
  UNION ALL SELECT 'image-generation', 'pricing_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 8040, 'Target user profiles the tool is designed for.', 'Zielgruppen-Nutzerprofile, fuer die das Tool konzipiert ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'image-generation', 'max_output_resolution', 'above_2048', 'Above 2048px', 'Ueber 2048px', 'neutral', 40
  ...
  UNION ALL SELECT 'image-generation', 'training_data_transparency', 'full_disclosure', 'Full disclosure', 'Vollstaendige Offenlegung', 'positive', 10
  ...
  UNION ALL SELECT 'image-generation', 'commercial_use_allowed', 'limited_commercial', 'Limited commercial use', 'Eingeschraenkte kommerzielle Nutzung', 'neutral', 20
  ...
  UNION ALL SELECT 'image-generation', 'commercial_use_allowed', 'license_dependent', 'License-dependent', 'Lizenzabhaengig', 'tradeoff', 40
  ...
  UNION ALL SELECT 'image-generation', 'output_ownership', 'varies_by_plan', 'Varies by plan', 'Abhaengig vom Tarif', 'tradeoff', 40
  ...
  UNION ALL SELECT 'image-generation', 'gdpr_data_processing_location', 'eu_primary', 'EU primary, non-EU fallback', 'EU primaer, Nicht-EU-Ausweichstandort', 'neutral', 20
  ...
  UNION ALL SELECT 'image-generation', 'prompt_logging', 'full_logging', 'Full logging', 'Vollstaendige Protokollierung', 'warning', 30
  ...
  UNION ALL SELECT 'image-generation', 'model_weights_availability', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 30
  ...
  UNION ALL SELECT 'image-generation', 'source_code_model', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 40
  ...
  UNION ALL SELECT 'image-generation', 'license_type', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 50
  ...
  UNION ALL SELECT 'image-generation', 'fit_profiles', 'artist', 'Artist', 'Kuenstler', 'neutral', 10
  ...
  UNION ALL SELECT 'image-generation', 'fit_profiles', 'enterprise', 'Enterprise', 'Grossunternehmen', 'neutral', 60
) AS d
