...
VALUES
  ('dns', 'resolver_infrastructure', 'Resolver Infrastructure', 'Resolver-Infrastruktur', 'Server network, geographic presence, and infrastructure operation model of the DNS resolver.', 'Servernetzwerk, geografische Praesenz und Betriebsmodell der Infrastruktur des DNS-Resolvers.', 100),
  ('dns', 'protocol_encryption', 'Protocol & Encryption', 'Protokoll & Verschluesselung', 'Supported DNS transport protocols, encryption standards, and DNSSEC validation.', 'Unterstuetzte DNS-Transportprotokolle, Verschluesselungsstandards und DNSSEC-Validierung.', 200),
  ...
  ('dns', 'platform_integration', 'Platform & Integration', 'Plattform & Integration', 'Supported operating systems, native apps, and network configuration options.', 'Unterstuetzte Betriebssysteme, native Apps und Netzwerkkonfigurationsoptionen.', 500),
  ('dns', 'performance_reliability', 'Performance & Reliability', 'Leistung & Zuverlaessigkeit', 'Response latency, uptime guarantees, rate limits, and EDNS Client Subnet behavior.', 'Antwortlatenz, Verfuegbarkeitsgarantien, Ratenlimits und EDNS-Client-Subnet-Verhalten.', 600),
  ...
  ('dns', 'account_pricing', 'Account & Pricing', 'Konto & Preise', 'Free tier availability, account requirements, pricing models, and payment methods.', 'Verfuegbarkeit kostenloser Stufen, Kontoanforderungen, Preismodelle und Zahlungsmethoden.', 800),
  ('dns', 'openness_governance_fit', 'Openness, Governance & Fit', 'Offenheit, Governance & Eignung', 'Source code openness, governance structure, self-hosting capability, and target user fit profiles.', 'Quellcode-Offenheit, Governance-Struktur, Self-Hosting-Faehigkeit und Zielgruppen-Eignungsprofile.', 900);
...
FROM (
  ...
  UNION ALL SELECT 'dns', 'resolver_infrastructure', 'anycast_network', 'Anycast network', 'Anycast-Netzwerk', 'boolean', 'beneficial', 'optional', 1020, 'Whether the resolver uses anycast routing to direct queries to the nearest server.', 'Ob der Resolver Anycast-Routing verwendet, um Anfragen an den naechsten Server zu leiten.'
  UNION ALL SELECT 'dns', 'resolver_infrastructure', 'server_regions', 'Server regions', 'Serverregionen', 'multi_enum', 'informational', 'multi_select', 1030, 'Geographic regions where the DNS resolver has server presence.', 'Geografische Regionen, in denen der DNS-Resolver Serverpraesenz hat.'
  ...
  UNION ALL SELECT 'dns', 'protocol_encryption', 'supported_protocols', 'Supported protocols', 'Unterstuetzte Protokolle', 'multi_enum', 'informational', 'multi_select', 2010, 'DNS transport protocols supported by the resolver for encrypted queries.', 'Vom Resolver unterstuetzte DNS-Transportprotokolle fuer verschluesselte Anfragen.'
  UNION ALL SELECT 'dns', 'protocol_encryption', 'default_dns_protocol', 'Default DNS protocol', 'Standard-DNS-Protokoll', 'enum', 'tradeoff', 'optional', 2020, 'The DNS protocol used by default when connecting to the resolver.', 'Das standardmaessig beim Verbinden mit dem Resolver verwendete DNS-Protokoll.'
  UNION ALL SELECT 'dns', 'protocol_encryption', 'tls_version', 'Minimum TLS version', 'Minimale TLS-Version', 'enum', 'beneficial', 'optional', 2030, 'The minimum TLS version accepted by the resolver for encrypted connections.', 'Die minimale TLS-Version, die der Resolver fuer verschluesselte Verbindungen akzeptiert.'
  UNION ALL SELECT 'dns', 'protocol_encryption', 'dnssec_validation', 'DNSSEC validation', 'DNSSEC-Validierung', 'boolean', 'beneficial', 'must_match', 2040, 'Whether the resolver performs DNSSEC validation to verify DNS response authenticity.', 'Ob der Resolver DNSSEC-Validierung durchfuehrt, um die Authentizitaet von DNS-Antworten zu pruefen.'
  UNION ALL SELECT 'dns', 'protocol_encryption', 'encrypted_sni', 'Encrypted Client Hello (ECH) support', 'Encrypted Client Hello (ECH)-Unterstuetzung', 'enum', 'beneficial', 'optional', 2050, 'Whether the resolver supports Encrypted Client Hello to protect SNI from eavesdropping.', 'Ob der Resolver Encrypted Client Hello unterstuetzt, um SNI vor Abhoeren zu schuetzen.'
  UNION ALL SELECT 'dns', 'privacy_logging', 'query_logging_policy', 'Query logging policy', 'Abfrageprotokollierungsrichtlinie', 'enum', 'risk', 'must_match', 3010, 'How the resolver handles logging of DNS queries and associated metadata.', 'Wie der Resolver die Protokollierung von DNS-Anfragen und zugehoerigen Metadaten handhabt.'
  ...
  UNION ALL SELECT 'dns', 'privacy_logging', 'independent_audit', 'Independent audit status', 'Unabhaengiger Audit-Status', 'enum', 'beneficial', 'optional', 3030, 'Whether the resolver has undergone independent privacy or security audits.', 'Ob der Resolver unabhaengige Datenschutz- oder Sicherheitsaudits durchlaufen hat.'
  UNION ALL SELECT 'dns', 'privacy_logging', 'jurisdiction', 'Legal jurisdiction', 'Rechtliche Zustaendigkeit', 'enum', 'tradeoff', 'optional', 3040, 'The legal jurisdiction under which the DNS resolver operator is incorporated.', 'Die rechtliche Zustaendigkeit, unter der der DNS-Resolver-Betreiber eingetragen ist.'
  UNION ALL SELECT 'dns', 'privacy_logging', 'transparency_report', 'Transparency report', 'Transparenzbericht', 'boolean', 'beneficial', 'optional', 3050, 'Whether the resolver operator publishes transparency reports on data requests.', 'Ob der Resolver-Betreiber Transparenzberichte ueber Datenanfragen veroeffentlicht.'
  ...
  UNION ALL SELECT 'dns', 'platform_integration', 'supported_platforms', 'Supported platforms', 'Unterstuetzte Plattformen', 'multi_enum', 'informational', 'multi_select', 5010, 'Operating systems and device types supported by the DNS resolver.', 'Vom DNS-Resolver unterstuetzte Betriebssysteme und Geraetetypen.'
  UNION ALL SELECT 'dns', 'platform_integration', 'native_app_available', 'Native app available', 'Native App verfuegbar', 'enum', 'beneficial', 'optional', 5020, 'Whether a dedicated application is available for device configuration.', 'Ob eine dedizierte Anwendung zur Geraetekonfiguration verfuegbar ist.'
  UNION ALL SELECT 'dns', 'platform_integration', 'router_configuration', 'Router configuration support', 'Router-Konfigurationsunterstuetzung', 'boolean', 'informational', 'optional', 5030, 'Whether the resolver provides guidance or support for router-level configuration.', 'Ob der Resolver Anleitungen oder Unterstuetzung fuer Router-Konfiguration bietet.'
  UNION ALL SELECT 'dns', 'platform_integration', 'browser_extension_available', 'Browser extension available', 'Browser-Erweiterung verfuegbar', 'boolean', 'beneficial', 'optional', 5040, 'Whether a browser extension is available for easy resolver configuration.', 'Ob eine Browser-Erweiterung fuer einfache Resolver-Konfiguration verfuegbar ist.'
  UNION ALL SELECT 'dns', 'platform_integration', 'network_level_setup', 'Network-level setup guide', 'Netzwerkweite Einrichtungsanleitung', 'boolean', 'informational', 'optional', 5050, 'Whether the resolver provides documentation for network-wide DNS setup.', 'Ob der Resolver Dokumentation fuer netzwerkweite DNS-Einrichtung bereitstellt.'
  UNION ALL SELECT 'dns', 'performance_reliability', 'average_latency_europe', 'Average latency (Europe)', 'Durchschnittliche Latenz (Europa)', 'enum', 'informational', 'optional', 6010, 'Typical DNS query response time measured from European locations.', 'Typische DNS-Anfrage-Antwortzeit, gemessen von europaeischen Standorten.'
  UNION ALL SELECT 'dns', 'performance_reliability', 'uptime_sla', 'Uptime SLA', 'Verfuegbarkeits-SLA', 'enum', 'beneficial', 'optional', 6020, 'The uptime guarantee or service level agreement offered by the resolver.', 'Die vom Resolver angebotene Verfuegbarkeitsgarantie oder Dienstguetevereinbarung.'
  ...
  UNION ALL SELECT 'dns', 'performance_reliability', 'ecs_support', 'EDNS Client Subnet (ECS) support', 'EDNS Client Subnet (ECS)-Unterstuetzung', 'enum', 'tradeoff', 'optional', 6040, 'Whether the resolver forwards client subnet information to authoritative servers for CDN optimization, trading privacy for performance.', 'Ob der Resolver Client-Subnetz-Informationen an autoritative Server zur CDN-Optimierung weiterleitet, wobei Datenschutz gegen Leistung getauscht wird.'
  UNION ALL SELECT 'dns', 'customization_features', 'custom_blocklists', 'Custom blocklists', 'Eigene Sperrlisten', 'boolean', 'beneficial', 'optional', 7010, 'Whether users can add custom domain blocklists to the resolver.', 'Ob Nutzer eigene Domain-Sperrlisten zum Resolver hinzufuegen koennen.'
  UNION ALL SELECT 'dns', 'customization_features', 'allowlisting', 'Allowlisting (whitelist)', 'Freigabeliste (Whitelist)', 'boolean', 'beneficial', 'optional', 7020, 'Whether users can exempt specific domains from blocking rules.', 'Ob Nutzer bestimmte Domains von Blockierungsregeln ausnehmen koennen.'
  UNION ALL SELECT 'dns', 'customization_features', 'query_analytics_dashboard', 'Query analytics dashboard', 'Abfrageanalyse-Dashboard', 'boolean', 'informational', 'optional', 7030, 'Whether a dashboard is available to view DNS query statistics and logs.', 'Ob ein Dashboard zur Anzeige von DNS-Abfragestatistiken und -protokollen verfuegbar ist.'
  UNION ALL SELECT 'dns', 'customization_features', 'multiple_profiles', 'Multiple DNS profiles', 'Mehrere DNS-Profile', 'boolean', 'informational', 'optional', 7040, 'Whether the resolver supports multiple configurations or filtering profiles.', 'Ob der Resolver mehrere Konfigurationen oder Filterprofile unterstuetzt.'
  UNION ALL SELECT 'dns', 'customization_features', 'api_access', 'API access', 'API-Zugang', 'boolean', 'beneficial', 'optional', 7050, 'Whether a programmatic API is available for managing resolver settings.', 'Ob eine programmatische API zur Verwaltung von Resolver-Einstellungen verfuegbar ist.'
  ...
  UNION ALL SELECT 'dns', 'account_pricing', 'free_tier_available', 'Free tier available', 'Kostenlose Stufe verfuegbar', 'boolean', 'informational', 'optional', 8010, 'Whether a free tier is available without payment or time limits.', 'Ob eine kostenlose Stufe ohne Zahlung oder Zeitbegrenzung verfuegbar ist.'
  ...
  UNION ALL SELECT 'dns', 'account_pricing', 'premium_pricing_model', 'Premium pricing model', 'Premium-Preismodell', 'enum', 'informational', 'optional', 8030, 'The pricing model for premium or paid features of the resolver.', 'Das Preismodell fuer Premium- oder kostenpflichtige Funktionen des Resolvers.'
  UNION ALL SELECT 'dns', 'account_pricing', 'cryptocurrency_payment', 'Cryptocurrency payment', 'Kryptowaehrungszahlung', 'boolean', 'beneficial', 'optional', 8040, 'Whether cryptocurrency is accepted as a payment method.', 'Ob Kryptowaehrung als Zahlungsmethode akzeptiert wird.'
  ...
  UNION ALL SELECT 'dns', 'openness_governance_fit', 'self_hostable', 'Self-hostable resolver', 'Self-Hosting-faehiger Resolver', 'boolean', 'beneficial', 'optional', 9030, 'Whether the resolver software can be self-hosted on private infrastructure.', 'Ob die Resolver-Software auf eigener Infrastruktur selbst gehostet werden kann.'
  UNION ALL SELECT 'dns', 'openness_governance_fit', 'bug_bounty_program', 'Bug bounty program', 'Bug-Bounty-Programm', 'boolean', 'beneficial', 'optional', 9040, 'Whether the resolver operator runs a security vulnerability reward program.', 'Ob der Resolver-Betreiber ein Sicherheitsluecken-Belohnungsprogramm betreibt.'
  UNION ALL SELECT 'dns', 'openness_governance_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9050, 'Common user profiles or use cases the DNS resolver is well suited for.', 'Typische Nutzerprofile oder Anwendungsfaelle, fuer die der DNS-Resolver gut geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'dns', 'server_regions', 'south_america', 'South America', 'Suedamerika', 'neutral', 40
  ...
  UNION ALL SELECT 'dns', 'supported_protocols', 'dns_over_https', 'DNS over HTTPS (DoH)', 'DNS ueber HTTPS (DoH)', 'neutral', 10
  UNION ALL SELECT 'dns', 'supported_protocols', 'dns_over_tls', 'DNS over TLS (DoT)', 'DNS ueber TLS (DoT)', 'neutral', 20
  UNION ALL SELECT 'dns', 'supported_protocols', 'dns_over_quic', 'DNS over QUIC (DoQ)', 'DNS ueber QUIC (DoQ)', 'neutral', 30
  UNION ALL SELECT 'dns', 'supported_protocols', 'dnscrypt', 'DNSCrypt', 'DNSCrypt', 'neutral', 40
  UNION ALL SELECT 'dns', 'supported_protocols', 'plain_dns', 'Plain DNS', 'Unverschluesseltes DNS', 'neutral', 50
  UNION ALL SELECT 'dns', 'supported_protocols', 'dns_over_https3', 'DNS over HTTP/3 (DoH3)', 'DNS ueber HTTP/3 (DoH3)', 'neutral', 60
  UNION ALL SELECT 'dns', 'default_dns_protocol', 'dns_over_https', 'DNS over HTTPS', 'DNS ueber HTTPS', 'neutral', 10
  UNION ALL SELECT 'dns', 'default_dns_protocol', 'dns_over_tls', 'DNS over TLS', 'DNS ueber TLS', 'neutral', 20
  UNION ALL SELECT 'dns', 'default_dns_protocol', 'dns_over_quic', 'DNS over QUIC', 'DNS ueber QUIC', 'neutral', 30
  UNION ALL SELECT 'dns', 'default_dns_protocol', 'plain_dns', 'Plain DNS', 'Unverschluesseltes DNS', 'neutral', 40
  UNION ALL SELECT 'dns', 'tls_version', 'tls_1_3_only', 'TLS 1.3 only', 'Nur TLS 1.3', 'positive', 10
  UNION ALL SELECT 'dns', 'tls_version', 'tls_1_2_and_above', 'TLS 1.2 and above', 'TLS 1.2 und hoeher', 'neutral', 20
  UNION ALL SELECT 'dns', 'tls_version', 'tls_1_1_and_above', 'TLS 1.1 and above', 'TLS 1.1 und hoeher', 'warning', 30
  UNION ALL SELECT 'dns', 'encrypted_sni', 'supported', 'Supported', 'Unterstuetzt', 'positive', 10
  UNION ALL SELECT 'dns', 'encrypted_sni', 'planned', 'Planned', 'Geplant', 'neutral', 20
  UNION ALL SELECT 'dns', 'encrypted_sni', 'not_supported', 'Not supported', 'Nicht unterstuetzt', 'tradeoff', 30
  ...
  UNION ALL SELECT 'dns', 'query_logging_policy', 'temporary_debug_logs', 'Temporary debug logs', 'Temporaere Debug-Protokolle', 'warning', 30
  UNION ALL SELECT 'dns', 'query_logging_policy', 'full_query_logging', 'Full query logging', 'Vollstaendige Abfrageprotokollierung', 'negative', 40
  UNION ALL SELECT 'dns', 'ip_address_retention', 'never_stored', 'Never stored', 'Nie gespeichert', 'positive', 10
  UNION ALL SELECT 'dns', 'ip_address_retention', 'hashed_or_truncated', 'Hashed or truncated', 'Gehasht oder gekuerzt', 'tradeoff', 20
  ...
  UNION ALL SELECT 'dns', 'malware_blocking', 'included_default', 'Included by default', 'Standardmaessig enthalten', 'positive', 10
  UNION ALL SELECT 'dns', 'malware_blocking', 'opt_in_profile', 'Opt-in profile', 'Opt-in-Profil', 'neutral', 20
  UNION ALL SELECT 'dns', 'malware_blocking', 'not_available', 'Not available', 'Nicht verfuegbar', 'tradeoff', 30
  UNION ALL SELECT 'dns', 'ad_tracker_blocking', 'included_default', 'Included by default', 'Standardmaessig enthalten', 'neutral', 10
  UNION ALL SELECT 'dns', 'ad_tracker_blocking', 'opt_in_profile', 'Opt-in profile', 'Opt-in-Profil', 'neutral', 20
  UNION ALL SELECT 'dns', 'ad_tracker_blocking', 'not_available', 'Not available', 'Nicht verfuegbar', 'neutral', 30
  UNION ALL SELECT 'dns', 'threat_intelligence_source', 'proprietary_curated', 'Proprietary curated', 'Proprietaer kuratiert', 'neutral', 10
  ...
  UNION ALL SELECT 'dns', 'native_app_available', 'full_app', 'Full app', 'Vollstaendige App', 'positive', 10
  ...
  UNION ALL SELECT 'dns', 'native_app_available', 'not_available', 'Not available', 'Nicht verfuegbar', 'tradeoff', 40
  ...
  UNION ALL SELECT 'dns', 'average_latency_europe', 'over_100ms', 'Over 100 ms', 'Ueber 100 ms', 'neutral', 40
  ...
  UNION ALL SELECT 'dns', 'uptime_sla', 'best_effort', 'Best effort', 'Nach bestem Bemuehen', 'tradeoff', 30
  ...
  UNION ALL SELECT 'dns', 'rate_limiting', 'generous_limits', 'Generous limits', 'Grosszuegige Limits', 'neutral', 20
  ...
  UNION ALL SELECT 'dns', 'source_model', 'source_available', 'Source available', 'Quellcode verfuegbar', 'neutral', 20
  UNION ALL SELECT 'dns', 'source_model', 'partial_open', 'Partial open', 'Teilweise offen', 'tradeoff', 30
  UNION ALL SELECT 'dns', 'source_model', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 40
  UNION ALL SELECT 'dns', 'governance_model', 'community_driven', 'Community driven', 'Community-getrieben', 'positive', 10
  UNION ALL SELECT 'dns', 'governance_model', 'foundation_backed', 'Foundation backed', 'Stiftungsunterstuetzt', 'positive', 20
  UNION ALL SELECT 'dns', 'governance_model', 'nonprofit', 'Nonprofit', 'Gemeinnuetzig', 'positive', 30
  UNION ALL SELECT 'dns', 'governance_model', 'corporate_independent', 'Corporate independent', 'Unabhaengiges Unternehmen', 'neutral', 40
