...
FROM (
  SELECT 'messaging' AS category_id, 'platform_access' AS group_key, 'cost_pricing' AS criterion_key, 'Cost/pricing' AS label_en, 'Kosten/Preismodell' AS label_de, 'enum' AS value_type, 'tradeoff' AS semantics, 'optional' AS filter_mode, 2060 AS sort_order, 'The documented consumer or team pricing model for normal use.' AS help_text_en, 'Das dokumentierte Preis- oder Kostenmodell für normale Nutzung.' AS help_text_de
  UNION ALL SELECT 'messaging', 'security_privacy', 'post_quantum_encryption', 'Post-quantum encryption', 'Post-Quantum-Verschlüsselung', 'boolean', 'beneficial', 'optional', 3080, 'Whether the messenger documents post-quantum or quantum-resistant protection for message encryption.', 'Ob der Messenger Post-Quantum- oder quantenresistente Schutzmechanismen für Nachrichtenverschlüsselung dokumentiert.'
  UNION ALL SELECT 'messaging', 'security_privacy', 'encryption_protocol_or_library', 'Encryption protocol/library', 'Verschlüsselungsprotokoll/-bibliothek', 'text', 'informational', 'none', 3090, 'The named protocol, cryptographic library, or encryption design documented for message protection.', 'Das dokumentierte Protokoll, die Kryptobibliothek oder das Verschlüsselungsdesign für Nachrichtenschutz.'
  UNION ALL SELECT 'messaging', 'security_privacy', 'proprietary_libraries', 'Proprietary libraries', 'Proprietäre Bibliotheken', 'boolean', 'risk', 'optional', 3100, 'Whether maintained clients include proprietary libraries or closed-source dependencies relevant to privacy or security review.', 'Ob gepflegte Clients proprietäre Bibliotheken oder geschlossene Abhängigkeiten enthalten, die für Datenschutz- oder Sicherheitsprüfung relevant sind.'
  UNION ALL SELECT 'messaging', 'security_privacy', 'tracker_or_ad_presence', 'Tracker/ad presence', 'Tracker-/Werbeanteile', 'enum', 'risk', 'optional', 3110, 'Documented presence of third-party trackers, advertising SDKs, behavioral ads, or explicit absence of them.', 'Dokumentierte Anwesenheit von Drittanbieter-Trackern, Werbe-SDKs, verhaltensbasierter Werbung oder deren ausdrückliche Abwesenheit.'
  UNION ALL SELECT 'messaging', 'security_privacy', 'latest_security_audit_year', 'Latest security audit year', 'Jahr des letzten Sicherheitsaudits', 'number', 'informational', 'range', 3120, 'The most recent year of a documented independent security audit, if one is documented.', 'Das neueste Jahr eines dokumentierten unabhängigen Sicherheitsaudits, sofern dokumentiert.'
  UNION ALL SELECT 'messaging', 'security_privacy', 'contact_key_change_warning', 'Key-change warning', 'Schlüsseländerungswarnung', 'boolean', 'beneficial', 'optional', 3130, 'Whether users are warned when a contact safety number, identity key, or verified device key changes.', 'Ob Nutzer gewarnt werden, wenn sich Sicherheitsnummer, Identitätsschlüssel oder verifizierter Geräteschlüssel eines Kontakts ändert.'
  UNION ALL SELECT 'messaging', 'architecture_control', 'client_source_openness', 'Client source openness', 'Quelloffenheit der Clients', 'enum', 'beneficial', 'optional', 4060, 'How openly the maintained client applications are licensed and published.', 'Wie offen die gepflegten Client-Anwendungen lizenziert und veröffentlicht sind.'
  UNION ALL SELECT 'messaging', 'architecture_control', 'server_source_openness', 'Server source openness', 'Quelloffenheit der Server', 'enum', 'beneficial', 'optional', 4070, 'How openly the server-side components are licensed and published, or whether no server is required.', 'Wie offen serverseitige Komponenten lizenziert und veröffentlicht sind oder ob kein Server erforderlich ist.'
  ...
  UNION ALL SELECT 'messaging', 'architecture_control', 'legal_jurisdiction', 'Legal jurisdiction', 'Rechtsraum', 'text', 'informational', 'none', 4090, 'The legal/operator jurisdiction relevant to the messenger service or project.', 'Der für Messenger-Dienst oder Projekt relevante Rechts- oder Betreiberstandort.'
  UNION ALL SELECT 'messaging', 'backup_export', 'automatic_backup', 'Automatic backup', 'Automatisches Backup', 'boolean', 'tradeoff', 'optional', 8050, 'Whether chat history or account data can be backed up automatically without a manual export step.', 'Ob Chatverlauf oder Kontodaten automatisch ohne manuellen Export gesichert werden können.'
  UNION ALL SELECT 'messaging', 'backup_export', 'backup_storage_location', 'Backup storage location', 'Backup-Speicherort', 'enum', 'tradeoff', 'optional', 8060, 'Where documented backups are stored or controlled when backup is available.', 'Wo dokumentierte Backups gespeichert oder kontrolliert werden, wenn Backup verfügbar ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'messaging', 'client_source_openness', 'full_open_source', 'Full open source', 'Vollständig Open Source', 'positive', 10
  ...
  UNION ALL SELECT 'messaging', 'client_source_openness', 'source_available_no_license', 'Source available, unclear license', 'Quellcode verfügbar, Lizenz unklar', 'warning', 30
  UNION ALL SELECT 'messaging', 'client_source_openness', 'proprietary', 'Proprietary', 'Proprietär', 'negative', 40
  UNION ALL SELECT 'messaging', 'server_source_openness', 'full_open_source', 'Full open source', 'Vollständig Open Source', 'positive', 10
  ...
  UNION ALL SELECT 'messaging', 'server_source_openness', 'source_available_no_license', 'Source available, unclear license', 'Quellcode verfügbar, Lizenz unklar', 'warning', 30
  UNION ALL SELECT 'messaging', 'server_source_openness', 'proprietary', 'Proprietary', 'Proprietär', 'negative', 40
  ...
  UNION ALL SELECT 'messaging', 'backup_storage_location', 'local_device', 'Local device', 'Lokales Gerät', 'positive', 10
