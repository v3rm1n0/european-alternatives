...
VALUES
  ('feed-reader', 'feed_support_import', 'Feed Support & Import', 'Feed-Unterstuetzung & Import', 'Feed protocol support, import and export formats, and subscription management capabilities.', 'Feed-Protokoll-Unterstuetzung, Import- und Export-Formate sowie Abonnementverwaltungsfunktionen.', 100),
  ...
  ('feed-reader', 'client_apps_access', 'Client Apps & Access', 'Client-Apps & Zugang', 'Available client applications, platform support, and access methods.', 'Verfuegbare Client-Anwendungen, Plattformunterstuetzung und Zugangsmoeglichkeiten.', 300),
  ('feed-reader', 'sync_sharing', 'Sync & Sharing', 'Synchronisation & Teilen', 'Cross-device synchronization, sharing capabilities, and collaboration features.', 'Geraeteuebergreifende Synchronisation, Freigabefunktionen und Zusammenarbeit.', 400),
  ...
  ('feed-reader', 'privacy_data', 'Privacy & Data', 'Privatsphaere & Daten', 'Account requirements, telemetry policies, data location, and feed fetching privacy.', 'Kontoanforderungen, Telemetrie-Richtlinien, Datenstandort und Feed-Abruf-Privatsphaere.', 600),
  ('feed-reader', 'openness_extensibility', 'Openness & Extensibility', 'Offenheit & Erweiterbarkeit', 'Source code availability, licensing terms, plugin support, and community ecosystem.', 'Quellcode-Verfuegbarkeit, Lizenzbedingungen, Plugin-Unterstuetzung und Community-Oekosystem.', 700),
  ...
  ('feed-reader', 'pricing_fit', 'Pricing & Fit', 'Preise & Eignung', 'Pricing structure, free tier availability, paid feature scope, and target audience fit.', 'Preisstruktur, Verfuegbarkeit kostenloser Versionen, Umfang kostenpflichtiger Funktionen und Zielgruppeneignung.', 900);
...
FROM (
  SELECT 'feed-reader' AS category_id, 'feed_support_import' AS group_key, 'supported_feed_formats' AS criterion_key, 'Supported feed formats' AS label_en, 'Unterstuetzte Feed-Formate' AS label_de, 'multi_enum' AS value_type, 'informational' AS semantics, 'multi_select' AS filter_mode, 1010 AS sort_order, 'The feed protocols and formats supported for subscription and reading.' AS help_text_en, 'Die unterstuetzten Feed-Protokolle und Formate fuer Abonnement und Lesen.' AS help_text_de
  ...
  UNION ALL SELECT 'feed-reader', 'feed_support_import', 'opml_support', 'OPML import / export', 'OPML-Import / -Export', 'enum', 'beneficial', 'optional', 1030, 'Whether subscriptions can be imported or exported using the OPML standard format.', 'Ob Abonnements im OPML-Standardformat importiert oder exportiert werden koennen.'
  UNION ALL SELECT 'feed-reader', 'feed_support_import', 'full_text_extraction', 'Full-text extraction', 'Volltext-Extraktion', 'boolean', 'beneficial', 'optional', 1040, 'Whether the reader can extract full article content from feeds that only provide summaries.', 'Ob der Reader den vollstaendigen Artikelinhalt aus Feeds extrahieren kann, die nur Zusammenfassungen liefern.'
  UNION ALL SELECT 'feed-reader', 'feed_support_import', 'feed_update_frequency', 'Feed update frequency', 'Feed-Aktualisierungshaeufigkeit', 'enum', 'informational', 'optional', 1050, 'How often the reader checks for new feed content and whether it supports push-based updates.', 'Wie oft der Reader auf neue Feed-Inhalte prueft und ob Push-basierte Aktualisierungen unterstuetzt werden.'
  UNION ALL SELECT 'feed-reader', 'feed_support_import', 'podcast_support', 'Podcast support', 'Podcast-Unterstuetzung', 'boolean', 'informational', 'optional', 1060, 'Whether the reader can subscribe to and play podcast audio feeds.', 'Ob der Reader Podcast-Audio-Feeds abonnieren und abspielen kann.'
  UNION ALL SELECT 'feed-reader', 'reading_experience', 'reading_view', 'Reading view', 'Lese-Ansicht', 'enum', 'informational', 'optional', 2010, 'The primary layout used to display articles and feed content in the reader interface.', 'Das primaere Layout zur Anzeige von Artikeln und Feed-Inhalten in der Reader-Oberflaeche.'
  UNION ALL SELECT 'feed-reader', 'reading_experience', 'offline_reading', 'Offline reading', 'Offline-Lesen', 'boolean', 'beneficial', 'optional', 2020, 'Whether articles can be saved for reading without an active internet connection.', 'Ob Artikel zum Lesen ohne aktive Internetverbindung gespeichert werden koennen.'
  ...
  UNION ALL SELECT 'feed-reader', 'reading_experience', 'tagging_labeling', 'Tagging / labeling', 'Tagging / Beschriftung', 'boolean', 'beneficial', 'optional', 2040, 'Whether articles can be tagged or labeled for personal organization beyond folders.', 'Ob Artikel fuer die persoenliche Organisation ueber Ordner hinaus mit Tags oder Labels versehen werden koennen.'
  UNION ALL SELECT 'feed-reader', 'reading_experience', 'folder_organization', 'Folder / category organization', 'Ordner- / Kategorieorganisation', 'boolean', 'beneficial', 'optional', 2050, 'Whether feeds can be grouped into folders or categories for structured browsing.', 'Ob Feeds zur strukturierten Navigation in Ordner oder Kategorien gruppiert werden koennen.'
  UNION ALL SELECT 'feed-reader', 'reading_experience', 'saved_articles', 'Saved articles / read-later', 'Gespeicherte Artikel / Spaeter lesen', 'boolean', 'beneficial', 'optional', 2060, 'Whether individual articles can be bookmarked or saved for later reading.', 'Ob einzelne Artikel als Lesezeichen gespeichert oder fuer spaeteres Lesen vorgemerkt werden koennen.'
  UNION ALL SELECT 'feed-reader', 'reading_experience', 'keyboard_shortcuts', 'Keyboard shortcuts', 'Tastenkuerzel', 'boolean', 'informational', 'optional', 2070, 'Whether the reader provides keyboard shortcuts for navigating and managing feeds.', 'Ob der Reader Tastenkuerzel zur Navigation und Verwaltung von Feeds bereitstellt.'
  UNION ALL SELECT 'feed-reader', 'client_apps_access', 'supported_platforms', 'Supported platforms', 'Unterstuetzte Plattformen', 'multi_enum', 'informational', 'multi_select', 3010, 'The operating systems and platforms with native or dedicated client applications.', 'Die Betriebssysteme und Plattformen mit nativen oder dedizierten Client-Anwendungen.'
  UNION ALL SELECT 'feed-reader', 'client_apps_access', 'web_interface', 'Web interface available', 'Weboberflaeche verfuegbar', 'boolean', 'beneficial', 'optional', 3020, 'Whether the reader offers a browser-based interface accessible from any device.', 'Ob der Reader eine browserbasierte Oberflaeche bietet, die von jedem Geraet zugaenglich ist.'
  UNION ALL SELECT 'feed-reader', 'client_apps_access', 'api_access', 'API access', 'API-Zugang', 'enum', 'informational', 'optional', 3030, 'The type of API available for third-party client integration and automation.', 'Die Art der verfuegbaren API fuer Drittanbieter-Client-Integration und Automatisierung.'
  UNION ALL SELECT 'feed-reader', 'client_apps_access', 'browser_extension', 'Browser extension', 'Browser-Erweiterung', 'boolean', 'informational', 'optional', 3040, 'Whether a browser extension is available for subscribing to feeds or reading articles.', 'Ob eine Browser-Erweiterung zum Abonnieren von Feeds oder Lesen von Artikeln verfuegbar ist.'
  UNION ALL SELECT 'feed-reader', 'client_apps_access', 'notification_support', 'Notification support', 'Benachrichtigungsunterstuetzung', 'multi_enum', 'informational', 'multi_select', 3050, 'The notification methods available for alerting users about new feed content.', 'Die verfuegbaren Benachrichtigungsmethoden zur Information ueber neue Feed-Inhalte.'
  UNION ALL SELECT 'feed-reader', 'sync_sharing', 'cross_device_sync', 'Cross-device sync', 'Geraeteuebergreifende Synchronisation', 'boolean', 'beneficial', 'must_match', 4010, 'Whether read status and subscriptions synchronize across multiple devices.', 'Ob Lesestatus und Abonnements ueber mehrere Geraete hinweg synchronisiert werden.'
  UNION ALL SELECT 'feed-reader', 'sync_sharing', 'sharing_options', 'Sharing options', 'Freigabeoptionen', 'multi_enum', 'informational', 'multi_select', 4020, 'The methods available for sharing articles with others or saving to external services.', 'Die verfuegbaren Methoden zum Teilen von Artikeln oder Speichern in externen Diensten.'
  UNION ALL SELECT 'feed-reader', 'sync_sharing', 'collaboration_features', 'Collaboration features', 'Zusammenarbeitsfunktionen', 'boolean', 'informational', 'optional', 4030, 'Whether the reader supports shared feeds or collaborative reading with other users.', 'Ob der Reader gemeinsame Feeds oder kollaboratives Lesen mit anderen Benutzern unterstuetzt.'
  ...
  UNION ALL SELECT 'feed-reader', 'filtering_automation', 'automation_integrations', 'Automation integrations', 'Automatisierungsintegrationen', 'multi_enum', 'informational', 'multi_select', 5030, 'The automation and workflow tools that can connect to the feed reader.', 'Die Automatisierungs- und Workflow-Tools, die mit dem Feed-Reader verbunden werden koennen.'
  UNION ALL SELECT 'feed-reader', 'filtering_automation', 'ai_features', 'AI features', 'KI-Funktionen', 'multi_enum', 'informational', 'multi_select', 5040, 'AI-powered features available for content processing and organization.', 'KI-gestuetzte Funktionen fuer die Inhaltsverarbeitung und Organisation.'
  UNION ALL SELECT 'feed-reader', 'privacy_data', 'account_requirement', 'Account requirement', 'Kontoanforderung', 'enum', 'risk', 'optional', 6010, 'Whether the reader requires creating an account to use its core features.', 'Ob der Reader die Erstellung eines Kontos fuer die Nutzung seiner Kernfunktionen erfordert.'
  UNION ALL SELECT 'feed-reader', 'privacy_data', 'telemetry_model', 'Telemetry model', 'Telemetriemodell', 'enum', 'risk', 'optional', 6020, 'How the reader handles collection and transmission of usage data.', 'Wie der Reader die Erfassung und Uebermittlung von Nutzungsdaten handhabt.'
  ...
  UNION ALL SELECT 'feed-reader', 'privacy_data', 'data_export_available', 'Data export available', 'Datenexport verfuegbar', 'boolean', 'beneficial', 'optional', 6050, 'Whether user data and subscriptions can be exported in a portable format.', 'Ob Benutzerdaten und Abonnements in einem portablen Format exportiert werden koennen.'
  UNION ALL SELECT 'feed-reader', 'openness_extensibility', 'source_model', 'Source code model', 'Quellcode-Modell', 'enum', 'tradeoff', 'optional', 7010, 'The openness of the reader source code and its availability to the public.', 'Die Offenheit des Reader-Quellcodes und seine oeffentliche Verfuegbarkeit.'
  ...
  UNION ALL SELECT 'feed-reader', 'openness_extensibility', 'plugin_theme_support', 'Plugin / theme support', 'Plugin- / Theme-Unterstuetzung', 'boolean', 'beneficial', 'optional', 7030, 'Whether the reader supports extending functionality through plugins or custom themes.', 'Ob der Reader die Erweiterung der Funktionalitaet durch Plugins oder benutzerdefinierte Themes unterstuetzt.'
  ...
  UNION ALL SELECT 'feed-reader', 'openness_extensibility', 'community_ecosystem', 'Community ecosystem', 'Community-Oekosystem', 'enum', 'informational', 'optional', 7050, 'The size and activity level of the community around the reader project.', 'Die Groesse und Aktivitaet der Community rund um das Reader-Projekt.'
  UNION ALL SELECT 'feed-reader', 'deployment_hosting', 'hosting_model', 'Hosting model', 'Hosting-Modell', 'enum', 'tradeoff', 'must_match', 8010, 'Whether the reader is available as a cloud service, self-hosted, or both.', 'Ob der Reader als Cloud-Dienst, selbstgehostet oder beides verfuegbar ist.'
  UNION ALL SELECT 'feed-reader', 'deployment_hosting', 'deployment_options', 'Deployment options', 'Bereitstellungsoptionen', 'multi_enum', 'informational', 'multi_select', 8020, 'The installation and deployment methods available for the reader.', 'Die verfuegbaren Installations- und Bereitstellungsmethoden fuer den Reader.'
  UNION ALL SELECT 'feed-reader', 'deployment_hosting', 'database_requirement', 'Database requirement', 'Datenbankanforderung', 'enum', 'informational', 'optional', 8030, 'The database backend required or supported by the reader for data storage.', 'Das Datenbank-Backend, das vom Reader fuer die Datenspeicherung benoetigt oder unterstuetzt wird.'
  UNION ALL SELECT 'feed-reader', 'deployment_hosting', 'minimum_server_requirements', 'Minimum server requirements', 'Minimale Serveranforderungen', 'enum', 'informational', 'optional', 8040, 'The minimum hardware tier needed to run the reader for basic operation.', 'Die minimale Hardware-Klasse, die fuer den grundlegenden Betrieb des Readers erforderlich ist.'
  ...
  UNION ALL SELECT 'feed-reader', 'pricing_fit', 'pricing_model', 'Pricing model', 'Preismodell', 'enum', 'informational', 'optional', 9010, 'The pricing structure of the reader software and any associated services.', 'Die Preisstruktur der Reader-Software und der zugehoerigen Dienste.'
  UNION ALL SELECT 'feed-reader', 'pricing_fit', 'free_tier_available', 'Free tier available', 'Kostenlose Version verfuegbar', 'boolean', 'informational', 'must_match', 9020, 'Whether a fully functional free version of the reader is available.', 'Ob eine voll funktionsfaehige kostenlose Version des Readers verfuegbar ist.'
  ...
  UNION ALL SELECT 'feed-reader', 'pricing_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9040, 'Target audience profiles the feed reader is best suited for.', 'Zielgruppenprofile, fuer die der Feed-Reader am besten geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'feed-reader', 'opml_support', 'not_supported', 'Not supported', 'Nicht unterstuetzt', 'neutral', 30
  ...
  UNION ALL SELECT 'feed-reader', 'api_access', 'public_documented', 'Public documented', 'Oeffentlich dokumentiert', 'neutral', 10
  ...
  UNION ALL SELECT 'feed-reader', 'sharing_options', 'public_shared_links', 'Public shared links', 'Oeffentliche geteilte Links', 'neutral', 30
  ...
  UNION ALL SELECT 'feed-reader', 'feed_categories_limit', 'generous', 'Generous (500+)', 'Grosszuegig (500+)', 'neutral', 20
  UNION ALL SELECT 'feed-reader', 'feed_categories_limit', 'moderate', 'Moderate (100-500)', 'Maessig (100-500)', 'neutral', 30
  ...
  UNION ALL SELECT 'feed-reader', 'ai_features', 'ai_translation', 'AI translation', 'KI-Uebersetzung', 'neutral', 40
  ...
  UNION ALL SELECT 'feed-reader', 'data_processing_location', 'eu_primary', 'EU primary', 'EU primaer', 'neutral', 30
  ...
  UNION ALL SELECT 'feed-reader', 'source_model', 'fully_open_source', 'Fully open source', 'Vollstaendig quelloffen', 'positive', 10
  UNION ALL SELECT 'feed-reader', 'source_model', 'source_available', 'Source available', 'Quellcode verfuegbar', 'neutral', 20
  UNION ALL SELECT 'feed-reader', 'source_model', 'partially_open', 'Partially open', 'Teilweise offen', 'neutral', 30
  UNION ALL SELECT 'feed-reader', 'source_model', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 40
  ...
  UNION ALL SELECT 'feed-reader', 'license_type', 'proprietary', 'Proprietary', 'Proprietaer', 'neutral', 50
  UNION ALL SELECT 'feed-reader', 'community_ecosystem', 'large_active', 'Large and active', 'Gross und aktiv', 'neutral', 10
  UNION ALL SELECT 'feed-reader', 'community_ecosystem', 'medium_active', 'Medium and active', 'Mittelgross und aktiv', 'neutral', 20
  ...
  UNION ALL SELECT 'feed-reader', 'database_requirement', 'multiple_supported', 'Multiple supported', 'Mehrere unterstuetzt', 'neutral', 50
  ...
  UNION ALL SELECT 'feed-reader', 'minimum_server_requirements', 'moderate', 'Moderate (VPS)', 'Maessig (VPS)', 'neutral', 30
  ...
  UNION ALL SELECT 'feed-reader', 'mobile_data_efficiency', 'moderate', 'Moderate', 'Maessig', 'neutral', 20
