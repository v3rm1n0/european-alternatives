...
VALUES
  ...
  ('analytics', 'data_storage_ownership', 'Data Storage & Ownership', 'Datenspeicherung & Eigentum', 'Data ownership rights, retention policies, and export capabilities.', 'Dateneigentumsrechte, Aufbewahrungsrichtlinien und Exportmoeglichkeiten.', 300),
  ...
  ('analytics', 'reporting_analysis', 'Reporting & Analysis', 'Berichte & Analyse', 'Reporting capabilities, analysis features, and data visualization options.', 'Berichtsfaehigkeiten, Analysefunktionen und Datenvisualisierungsoptionen.', 500),
  ('analytics', 'integration_implementation', 'Integration & Implementation', 'Integration & Implementierung', 'Implementation methods, APIs, plugins, and migration support.', 'Implementierungsmethoden, APIs, Plugins und Migrationsunterstuetzung.', 600),
...
FROM (
  ...
  UNION ALL SELECT 'analytics', 'data_collection', 'cookie_requirement', 'Cookie requirement', 'Cookie-Anforderung', 'enum', 'risk', 'must_match', 1020, 'Whether the analytics tool requires cookies to function.', 'Ob das Analysetool Cookies zum Funktionieren benoetigt.'
  ...
  UNION ALL SELECT 'analytics', 'data_collection', 'cross_domain_tracking', 'Cross-domain tracking', 'Cross-Domain-Tracking', 'boolean', 'informational', 'optional', 1040, 'Whether the tool supports tracking users across multiple domains.', 'Ob das Tool das Tracking von Nutzern ueber mehrere Domains hinweg unterstuetzt.'
  UNION ALL SELECT 'analytics', 'data_collection', 'event_tracking', 'Custom event tracking', 'Benutzerdefiniertes Event-Tracking', 'boolean', 'informational', 'optional', 1050, 'Whether custom events beyond page views can be tracked.', 'Ob benutzerdefinierte Ereignisse ueber Seitenaufrufe hinaus erfasst werden koennen.'
  ...
  UNION ALL SELECT 'analytics', 'privacy_compliance', 'data_residency_options', 'Data residency options', 'Optionen fuer Datenresidenz', 'multi_enum', 'tradeoff', 'multi_select', 2030, 'Geographic regions where analytics data can be stored.', 'Geografische Regionen, in denen Analysedaten gespeichert werden koennen.'
  UNION ALL SELECT 'analytics', 'privacy_compliance', 'privacy_audit', 'Independent privacy audit', 'Unabhaengiges Datenschutz-Audit', 'enum', 'beneficial', 'optional', 2040, 'Whether the analytics tool has undergone independent privacy audits.', 'Ob das Analysetool unabhaengige Datenschutz-Audits durchlaufen hat.'
  UNION ALL SELECT 'analytics', 'privacy_compliance', 'data_processor_agreement', 'Data processor agreement (DPA)', 'Auftragsverarbeitungsvertrag (AVV)', 'boolean', 'beneficial', 'optional', 2050, 'Whether a data processor agreement is available for GDPR compliance.', 'Ob ein Auftragsverarbeitungsvertrag fuer die DSGVO-Compliance verfuegbar ist.'
  UNION ALL SELECT 'analytics', 'data_storage_ownership', 'data_ownership_model', 'Data ownership model', 'Dateneigentumsmodell', 'enum', 'tradeoff', 'optional', 3010, 'Who owns the collected analytics data and associated processing rights.', 'Wem die gesammelten Analysedaten und zugehoerigen Verarbeitungsrechte gehoeren.'
  ...
  UNION ALL SELECT 'analytics', 'data_storage_ownership', 'raw_data_access', 'Raw data access', 'Rohdatenzugriff', 'enum', 'beneficial', 'optional', 3030, 'Whether raw analytics data can be accessed directly by the customer.', 'Ob Roh-Analysedaten direkt vom Kunden abgerufen werden koennen.'
  UNION ALL SELECT 'analytics', 'data_storage_ownership', 'data_export_format', 'Data export format', 'Datenexportformat', 'multi_enum', 'informational', 'multi_select', 3040, 'Formats available for exporting analytics data.', 'Verfuegbare Formate fuer den Export von Analysedaten.'
  UNION ALL SELECT 'analytics', 'data_storage_ownership', 'data_deletion_capability', 'Data deletion on request', 'Datenloeschung auf Anfrage', 'boolean', 'beneficial', 'optional', 3050, 'Whether analytics data can be deleted on request.', 'Ob Analysedaten auf Anfrage geloescht werden koennen.'
  UNION ALL SELECT 'analytics', 'hosting_deployment', 'hosting_model', 'Hosting model', 'Hosting-Modell', 'enum', 'tradeoff', 'must_match', 4010, 'The hosting options available for the analytics tool.', 'Die verfuegbaren Hosting-Optionen fuer das Analysetool.'
  UNION ALL SELECT 'analytics', 'hosting_deployment', 'self_hosting_available', 'Self-hosting available', 'Self-Hosting verfuegbar', 'boolean', 'beneficial', 'must_match', 4020, 'Whether the analytics tool can be self-hosted on private infrastructure.', 'Ob das Analysetool auf eigener Infrastruktur selbst gehostet werden kann.'
  UNION ALL SELECT 'analytics', 'hosting_deployment', 'cloud_regions', 'Cloud hosting regions', 'Cloud-Hosting-Regionen', 'multi_enum', 'informational', 'multi_select', 4030, 'Geographic regions where cloud hosting is available.', 'Geografische Regionen, in denen Cloud-Hosting verfuegbar ist.'
  UNION ALL SELECT 'analytics', 'hosting_deployment', 'docker_deployment', 'Docker deployment', 'Docker-Bereitstellung', 'boolean', 'beneficial', 'optional', 4040, 'Whether the analytics tool supports deployment via Docker containers.', 'Ob das Analysetool die Bereitstellung ueber Docker-Container unterstuetzt.'
  ...
  UNION ALL SELECT 'analytics', 'reporting_analysis', 'funnel_analysis', 'Funnel analysis', 'Trichteranalyse', 'boolean', 'informational', 'optional', 5020, 'Whether conversion funnel analysis is supported.', 'Ob Conversion-Trichteranalyse unterstuetzt wird.'
  UNION ALL SELECT 'analytics', 'reporting_analysis', 'segmentation', 'User segmentation', 'Nutzersegmentierung', 'boolean', 'informational', 'optional', 5030, 'Whether visitor data can be segmented by attributes or behavior.', 'Ob Besucherdaten nach Attributen oder Verhalten segmentiert werden koennen.'
  UNION ALL SELECT 'analytics', 'reporting_analysis', 'custom_dashboards', 'Custom dashboards', 'Benutzerdefinierte Dashboards', 'boolean', 'informational', 'optional', 5040, 'Whether custom dashboards can be created for data visualization.', 'Ob benutzerdefinierte Dashboards zur Datenvisualisierung erstellt werden koennen.'
  UNION ALL SELECT 'analytics', 'reporting_analysis', 'goal_tracking', 'Goal / conversion tracking', 'Ziel- / Conversion-Tracking', 'boolean', 'informational', 'optional', 5050, 'Whether goal and conversion tracking is supported.', 'Ob Ziel- und Conversion-Tracking unterstuetzt wird.'
  UNION ALL SELECT 'analytics', 'integration_implementation', 'implementation_method', 'Implementation method', 'Implementierungsmethode', 'multi_enum', 'informational', 'multi_select', 6010, 'Methods available for implementing the analytics tracking code.', 'Verfuegbare Methoden zur Implementierung des Analyse-Tracking-Codes.'
  UNION ALL SELECT 'analytics', 'integration_implementation', 'api_available', 'Public API', 'Oeffentliche API', 'boolean', 'beneficial', 'optional', 6020, 'Whether a public API is available for programmatic data access.', 'Ob eine oeffentliche API fuer programmatischen Datenzugriff verfuegbar ist.'
  UNION ALL SELECT 'analytics', 'integration_implementation', 'cms_plugins', 'CMS plugins available', 'CMS-Plugins verfuegbar', 'multi_enum', 'informational', 'multi_select', 6030, 'Content management systems with official plugin support.', 'Content-Management-Systeme mit offizieller Plugin-Unterstuetzung.'
  UNION ALL SELECT 'analytics', 'integration_implementation', 'tag_manager_support', 'Tag manager support', 'Tag-Manager-Unterstuetzung', 'boolean', 'informational', 'optional', 6040, 'Whether integration via tag managers is supported.', 'Ob die Integration ueber Tag-Manager unterstuetzt wird.'
  UNION ALL SELECT 'analytics', 'integration_implementation', 'import_from_ga', 'Import from Google Analytics', 'Import aus Google Analytics', 'boolean', 'informational', 'optional', 6050, 'Whether historical data can be imported from Google Analytics.', 'Ob historische Daten aus Google Analytics importiert werden koennen.'
  UNION ALL SELECT 'analytics', 'scale_pricing', 'free_tier_available', 'Free tier available', 'Kostenlose Stufe verfuegbar', 'boolean', 'informational', 'optional', 7010, 'Whether a free tier is available without payment requirements.', 'Ob eine kostenlose Stufe ohne Zahlungsanforderungen verfuegbar ist.'
  ...
  UNION ALL SELECT 'analytics', 'scale_pricing', 'pricing_metric', 'Pricing metric', 'Preismetrik', 'enum', 'tradeoff', 'optional', 7030, 'The primary metric used to calculate pricing.', 'Die primaere Metrik zur Berechnung der Preise.'
  ...
  UNION ALL SELECT 'analytics', 'scale_pricing', 'team_seats', 'Team seats', 'Team-Plaetze', 'enum', 'informational', 'optional', 7050, 'The number of team member seats included.', 'Die Anzahl der enthaltenen Team-Mitglieder-Plaetze.'
  UNION ALL SELECT 'analytics', 'features_ux', 'heatmaps', 'Heatmaps', 'Heatmaps', 'boolean', 'informational', 'optional', 8010, 'Whether click and scroll heatmaps are available.', 'Ob Klick- und Scroll-Heatmaps verfuegbar sind.'
  UNION ALL SELECT 'analytics', 'features_ux', 'session_recordings', 'Session recordings', 'Sitzungsaufzeichnungen', 'boolean', 'tradeoff', 'optional', 8020, 'Whether visitor session recordings are available for behavioral analysis.', 'Ob Besucher-Sitzungsaufzeichnungen zur Verhaltensanalyse verfuegbar sind.'
  UNION ALL SELECT 'analytics', 'features_ux', 'ab_testing', 'A/B testing', 'A/B-Tests', 'boolean', 'informational', 'optional', 8030, 'Whether built-in A/B testing functionality is available.', 'Ob integrierte A/B-Test-Funktionalitaet verfuegbar ist.'
  UNION ALL SELECT 'analytics', 'features_ux', 'user_flow_analysis', 'User flow analysis', 'Nutzerflussanalyse', 'boolean', 'informational', 'optional', 8040, 'Whether user flow and navigation path analysis is supported.', 'Ob Nutzerfluss- und Navigationspfad-Analyse unterstuetzt wird.'
  UNION ALL SELECT 'analytics', 'features_ux', 'lightweight_script', 'Lightweight tracking script', 'Leichtgewichtiges Tracking-Skript', 'enum', 'beneficial', 'optional', 8050, 'The approximate size of the tracking script loaded in the browser.', 'Die ungefaehre Groesse des im Browser geladenen Tracking-Skripts.'
  ...
  UNION ALL SELECT 'analytics', 'openness_governance_fit', 'community_size', 'Community size', 'Community-Groesse', 'enum', 'informational', 'optional', 9030, 'The size of the open-source community around the analytics tool.', 'Die Groesse der Open-Source-Community rund um das Analysetool.'
  UNION ALL SELECT 'analytics', 'openness_governance_fit', 'bug_bounty_program', 'Bug bounty program', 'Bug-Bounty-Programm', 'boolean', 'beneficial', 'optional', 9040, 'Whether the analytics tool operator runs a security vulnerability reward program.', 'Ob der Analysetool-Betreiber ein Sicherheitsluecken-Belohnungsprogramm betreibt.'
  UNION ALL SELECT 'analytics', 'openness_governance_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9050, 'Common user profiles or use cases the analytics tool is well suited for.', 'Typische Nutzerprofile oder Anwendungsfaelle, fuer die das Analysetool gut geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'analytics', 'consent_required', 'no_consent_needed', 'No consent needed', 'Keine Einwilligung noetig', 'positive', 10
  ...
  UNION ALL SELECT 'analytics', 'data_ownership_model', 'vendor_retains_rights', 'Vendor retains rights', 'Anbieter behaelt Rechte', 'warning', 30
  ...
  UNION ALL SELECT 'analytics', 'raw_data_access', 'limited_access', 'Limited access', 'Eingeschraenkter Zugang', 'neutral', 20
  ...
  UNION ALL SELECT 'analytics', 'monthly_pageview_limit', 'over_100k', 'Over 100k', 'Ueber 100k', 'neutral', 20
  ...
  UNION ALL SELECT 'analytics', 'team_seats', 'generous', 'Generous (10+)', 'Grosszuegig (10+)', 'neutral', 20
  ...
  UNION ALL SELECT 'analytics', 'lightweight_script', 'over_100kb', 'Over 100 KB', 'Ueber 100 KB', 'tradeoff', 40
  UNION ALL SELECT 'analytics', 'source_model', 'open_source', 'Open source', 'Open Source', 'positive', 10
  UNION ALL SELECT 'analytics', 'source_model', 'source_available', 'Source available', 'Quellcode verfuegbar', 'neutral', 20
  UNION ALL SELECT 'analytics', 'source_model', 'partial_open', 'Partial open', 'Teilweise offen', 'tradeoff', 30
  UNION ALL SELECT 'analytics', 'source_model', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 40
  UNION ALL SELECT 'analytics', 'governance_model', 'community_driven', 'Community driven', 'Community-getrieben', 'positive', 10
  UNION ALL SELECT 'analytics', 'governance_model', 'foundation_backed', 'Foundation backed', 'Stiftungsunterstuetzt', 'positive', 20
  UNION ALL SELECT 'analytics', 'governance_model', 'nonprofit', 'Nonprofit', 'Gemeinnuetzig', 'positive', 30
  UNION ALL SELECT 'analytics', 'governance_model', 'corporate_independent', 'Corporate independent', 'Unabhaengiges Unternehmen', 'neutral', 40
  ...
  UNION ALL SELECT 'analytics', 'community_size', 'large', 'Large (10k+ stars)', 'Gross (10k+ Sterne)', 'neutral', 10
