...
VALUES
  ('virtualization', 'hypervisor_architecture', 'Hypervisor & Architecture', 'Hypervisor & Architektur', 'Hypervisor type, virtualization technology, and supported CPU architectures.', 'Hypervisor-Typ, Virtualisierungstechnologie und unterstützte CPU-Architekturen.', 100),
  ('virtualization', 'guest_workload_support', 'Guest OS & Workload Support', 'Gast-Betriebssysteme & Workload-Unterstützung', 'Supported guest operating systems, container integration, VM templates, and nested virtualization.', 'Unterstützte Gast-Betriebssysteme, Container-Integration, VM-Vorlagen und verschachtelte Virtualisierung.', 200),
  ('virtualization', 'performance_hardware', 'Performance & Hardware Features', 'Leistung & Hardware-Funktionen', 'GPU passthrough, hardware acceleration, live migration, overcommitment, and vCPU limits.', 'GPU-Passthrough, Hardwarebeschleunigung, Live-Migration, Überzeichnung und vCPU-Limits.', 300),
  ('virtualization', 'management_administration', 'Management & Administration', 'Verwaltung & Administration', 'Management interfaces, cluster support, high availability, API access, and backup integration.', 'Verwaltungsoberflächen, Clusterunterstützung, Hochverfügbarkeit, API-Zugang und Backup-Integration.', 400),
  ('virtualization', 'networking_connectivity', 'Networking & Connectivity', 'Netzwerk & Konnektivität', 'Virtual networking modes, VLAN tagging, and software-defined networking capabilities.', 'Virtuelle Netzwerkmodi, VLAN-Tagging und Software-definierte Netzwerkfähigkeiten.', 500),
  ('virtualization', 'storage_snapshots', 'Storage & Snapshots', 'Speicher & Snapshots', 'Storage backends, snapshot capabilities, disk formats, thin provisioning, and live resize.', 'Speicher-Backends, Snapshot-Fähigkeiten, Festplattenformate, Thin Provisioning und Live-Resize.', 600),
  ('virtualization', 'privacy_data_control', 'Privacy & Data Control', 'Privatsphäre & Datenkontrolle', 'Telemetry collection, data processing location, and data export capabilities.', 'Telemetrie-Erfassung, Datenverarbeitungsstandort und Datenexport-Möglichkeiten.', 700),
  ('virtualization', 'openness_deployment', 'Openness & Deployment', 'Offenheit & Bereitstellung', 'Source code availability, licensing, self-hosting capability, and deployment models.', 'Quellcode-Verfügbarkeit, Lizenzierung, Selbsthosting-Fähigkeit und Bereitstellungsmodelle.', 800),
  ('virtualization', 'pricing_fit', 'Pricing & Fit', 'Preise & Eignung', 'Pricing structure, free tier availability, and target use case profiles.', 'Preisstruktur, Verfügbarkeit kostenloser Versionen und Zielgruppen-Einsatzprofile.', 900);
...
FROM (
  ...
  UNION ALL SELECT 'virtualization', 'hypervisor_architecture', 'cpu_architecture_support', 'CPU architecture support', 'CPU-Architektur-Unterstützung', 'multi_enum', 'informational', 'multi_select', 1030, 'Which CPU instruction set architectures the hypervisor supports for running virtual machines.', 'Welche CPU-Befehlssatzarchitekturen der Hypervisor für den Betrieb virtueller Maschinen unterstützt.'
  UNION ALL SELECT 'virtualization', 'guest_workload_support', 'guest_os_support', 'Guest OS support', 'Gastbetriebssystem-Unterstützung', 'multi_enum', 'informational', 'multi_select', 2010, 'Which operating system families can run as guest virtual machines on this hypervisor.', 'Welche Betriebssystemfamilien als virtuelle Gastmaschinen auf diesem Hypervisor ausgeführt werden können.'
  UNION ALL SELECT 'virtualization', 'guest_workload_support', 'container_support', 'Container support', 'Container-Unterstützung', 'boolean', 'informational', 'optional', 2020, 'Whether the platform supports running container workloads alongside or instead of full virtual machines.', 'Ob die Plattform Container-Workloads neben oder anstelle vollständiger virtueller Maschinen unterstützt.'
  UNION ALL SELECT 'virtualization', 'guest_workload_support', 'vm_template_library', 'VM template library', 'VM-Vorlagenbibliothek', 'boolean', 'beneficial', 'optional', 2030, 'Whether pre-built virtual machine templates are available for quick deployment of common operating systems.', 'Ob vorgefertigte VM-Vorlagen für die schnelle Bereitstellung gängiger Betriebssysteme verfügbar sind.'
  UNION ALL SELECT 'virtualization', 'guest_workload_support', 'nested_virtualization', 'Nested virtualization', 'Verschachtelte Virtualisierung', 'boolean', 'informational', 'optional', 2040, 'Whether running a hypervisor inside a virtual machine is supported for development and testing purposes.', 'Ob das Ausführen eines Hypervisors innerhalb einer virtuellen Maschine für Entwicklungs- und Testzwecke unterstützt wird.'
  UNION ALL SELECT 'virtualization', 'performance_hardware', 'gpu_passthrough', 'GPU passthrough', 'GPU-Durchreichung', 'boolean', 'beneficial', 'optional', 3010, 'Whether physical GPU devices can be passed through directly to a virtual machine for hardware-accelerated graphics.', 'Ob physische GPU-Geräte direkt an eine virtuelle Maschine für hardwarebeschleunigte Grafik durchgereicht werden können.'
  UNION ALL SELECT 'virtualization', 'performance_hardware', 'hardware_acceleration', 'Hardware acceleration', 'Hardwarebeschleunigung', 'enum', 'informational', 'optional', 3020, 'The hardware-assisted virtualization extensions used by the hypervisor for improved performance.', 'Die hardwaregestützten Virtualisierungserweiterungen, die der Hypervisor für verbesserte Leistung nutzt.'
  UNION ALL SELECT 'virtualization', 'performance_hardware', 'live_migration', 'Live migration', 'Live-Migration', 'boolean', 'beneficial', 'optional', 3030, 'Whether running virtual machines can be moved between physical hosts without downtime.', 'Ob laufende virtuelle Maschinen ohne Ausfallzeit zwischen physischen Hosts verschoben werden können.'
  UNION ALL SELECT 'virtualization', 'performance_hardware', 'resource_overcommit', 'Resource overcommitment', 'Ressourcenüberzeichnung', 'boolean', 'informational', 'optional', 3040, 'Whether more virtual CPU or memory resources can be allocated than physically available on the host.', 'Ob mehr virtuelle CPU- oder Speicherressourcen zugewiesen werden können als physisch auf dem Host verfügbar sind.'
  UNION ALL SELECT 'virtualization', 'performance_hardware', 'max_vcpu_per_vm', 'Max vCPUs per VM', 'Maximale vCPUs pro VM', 'enum', 'informational', 'optional', 3050, 'The maximum number of virtual CPU cores that can be assigned to a single virtual machine.', 'Die maximale Anzahl virtueller CPU-Kerne, die einer einzelnen virtuellen Maschine zugewiesen werden können.'
  UNION ALL SELECT 'virtualization', 'management_administration', 'management_interface', 'Management interface', 'Verwaltungsoberfläche', 'multi_enum', 'informational', 'multi_select', 4010, 'The interfaces available for managing the hypervisor and its virtual machines.', 'Die verfügbaren Oberflächen zur Verwaltung des Hypervisors und seiner virtuellen Maschinen.'
  UNION ALL SELECT 'virtualization', 'management_administration', 'cluster_management', 'Cluster management', 'Clusterverwaltung', 'boolean', 'beneficial', 'optional', 4020, 'Whether multiple physical hosts can be managed as a unified cluster from a single interface.', 'Ob mehrere physische Hosts als einheitlicher Cluster über eine einzige Oberfläche verwaltet werden können.'
  UNION ALL SELECT 'virtualization', 'management_administration', 'high_availability', 'High availability', 'Hochverfügbarkeit', 'boolean', 'beneficial', 'optional', 4030, 'Whether the platform provides automatic failover and restart of virtual machines when a host fails.', 'Ob die Plattform automatisches Failover und Neustart virtueller Maschinen bei Hostausfall bereitstellt.'
  UNION ALL SELECT 'virtualization', 'management_administration', 'api_available', 'API available', 'API verfügbar', 'boolean', 'beneficial', 'optional', 4040, 'Whether a programmable API is available for automating virtual machine and infrastructure management.', 'Ob eine programmierbare API für die Automatisierung der Verwaltung virtueller Maschinen und Infrastruktur verfügbar ist.'
  UNION ALL SELECT 'virtualization', 'management_administration', 'backup_integration', 'Backup integration', 'Backup-Integration', 'boolean', 'beneficial', 'optional', 4050, 'Whether the platform offers built-in backup tools or integration with third-party backup solutions.', 'Ob die Plattform integrierte Backup-Werkzeuge oder Integration mit Backup-Lösungen von Drittanbietern bietet.'
  UNION ALL SELECT 'virtualization', 'networking_connectivity', 'virtual_networking', 'Virtual networking modes', 'Virtuelle Netzwerkmodi', 'multi_enum', 'informational', 'multi_select', 5010, 'The virtual network connection modes available for guest virtual machines.', 'Die verfügbaren virtuellen Netzwerkverbindungsmodi für virtuelle Gastmaschinen.'
  UNION ALL SELECT 'virtualization', 'networking_connectivity', 'vlan_support', 'VLAN support', 'VLAN-Unterstützung', 'boolean', 'informational', 'optional', 5020, 'Whether the platform supports IEEE 802.1Q VLAN tagging for network traffic segmentation.', 'Ob die Plattform IEEE 802.1Q VLAN-Tagging zur Segmentierung des Netzwerkverkehrs unterstützt.'
  UNION ALL SELECT 'virtualization', 'networking_connectivity', 'sdn_support', 'Software-defined networking', 'Software-definiertes Netzwerk', 'boolean', 'informational', 'optional', 5030, 'Whether the platform integrates with software-defined networking controllers for advanced network management.', 'Ob die Plattform mit Software-definierten Netzwerk-Controllern für erweitertes Netzwerkmanagement integriert ist.'
  UNION ALL SELECT 'virtualization', 'storage_snapshots', 'storage_backends', 'Storage backends', 'Speicher-Backends', 'multi_enum', 'informational', 'multi_select', 6010, 'The storage technologies and protocols supported for storing virtual machine disk images.', 'Die Speichertechnologien und Protokolle, die für die Speicherung von VM-Festplattenabbildern unterstützt werden.'
  UNION ALL SELECT 'virtualization', 'storage_snapshots', 'snapshot_support', 'Snapshot support', 'Snapshot-Unterstützung', 'boolean', 'beneficial', 'optional', 6020, 'Whether point-in-time snapshots of virtual machine state and disk can be created and restored.', 'Ob zeitpunktbezogene Snapshots des VM-Zustands und der Festplatte erstellt und wiederhergestellt werden können.'
  UNION ALL SELECT 'virtualization', 'storage_snapshots', 'disk_format_support', 'Disk format support', 'Festplattenformat-Unterstützung', 'multi_enum', 'informational', 'multi_select', 6030, 'The virtual disk image formats that can be used or imported by the hypervisor.', 'Die virtuellen Festplattenabbildformate, die vom Hypervisor verwendet oder importiert werden können.'
  UNION ALL SELECT 'virtualization', 'storage_snapshots', 'thin_provisioning', 'Thin provisioning', 'Thin Provisioning', 'boolean', 'informational', 'optional', 6040, 'Whether disk space is allocated on demand rather than reserved upfront for virtual machine storage.', 'Ob Speicherplatz bei Bedarf zugewiesen wird, anstatt im Voraus für den VM-Speicher reserviert zu werden.'
  UNION ALL SELECT 'virtualization', 'storage_snapshots', 'storage_live_resize', 'Storage live resize', 'Speicher-Live-Größenänderung', 'boolean', 'informational', 'optional', 6050, 'Whether virtual disk volumes can be expanded while the virtual machine is still running.', 'Ob virtuelle Festplattenvolumes erweitert werden können, während die virtuelle Maschine noch läuft.'
  UNION ALL SELECT 'virtualization', 'privacy_data_control', 'telemetry_model', 'Telemetry model', 'Telemetriemodell', 'enum', 'risk', 'optional', 7010, 'How the virtualization platform collects and transmits usage data and analytics.', 'Wie die Virtualisierungsplattform Nutzungsdaten und Analysen erfasst und überträgt.'
  ...
  UNION ALL SELECT 'virtualization', 'privacy_data_control', 'data_export_available', 'Data export available', 'Datenexport verfügbar', 'boolean', 'beneficial', 'optional', 7030, 'Whether VM configurations and metadata can be exported in a portable format for migration.', 'Ob VM-Konfigurationen und Metadaten in einem portablen Format für die Migration exportiert werden können.'
  UNION ALL SELECT 'virtualization', 'openness_deployment', 'source_model', 'Source code model', 'Quellcode-Modell', 'enum', 'tradeoff', 'optional', 8010, 'The availability and openness of the virtualization platform source code.', 'Die Verfügbarkeit und Offenheit des Quellcodes der Virtualisierungsplattform.'
  ...
  UNION ALL SELECT 'virtualization', 'pricing_fit', 'pricing_model', 'Pricing model', 'Preismodell', 'enum', 'informational', 'optional', 9010, 'The pricing structure for obtaining and using the virtualization platform.', 'Die Preisstruktur für den Erwerb und die Nutzung der Virtualisierungsplattform.'
  UNION ALL SELECT 'virtualization', 'pricing_fit', 'free_tier_available', 'Free tier available', 'Kostenlose Version verfügbar', 'boolean', 'informational', 'must_match', 9020, 'Whether a functional free version of the platform is available without payment or time limits.', 'Ob eine funktionsfähige kostenlose Version der Plattform ohne Bezahlung oder Zeitlimits verfügbar ist.'
  UNION ALL SELECT 'virtualization', 'pricing_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9030, 'Target use case profiles the platform is best suited for based on features and scale.', 'Zielgruppen-Einsatzprofile, für die die Plattform basierend auf Funktionen und Skalierung am besten geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'virtualization', 'virtualization_technology', 'hardware_assisted', 'Hardware-assisted', 'Hardwaregestützt', 'neutral', 40
  ...
  UNION ALL SELECT 'virtualization', 'management_interface', 'web_ui', 'Web UI', 'Web-Oberfläche', 'neutral', 10
  ...
  UNION ALL SELECT 'virtualization', 'virtual_networking', 'bridged', 'Bridged', 'Netzwerkbrücke', 'neutral', 20
  ...
  UNION ALL SELECT 'virtualization', 'data_processing_location', 'eu_primary', 'EU primary', 'EU primär', 'neutral', 20
  UNION ALL SELECT 'virtualization', 'data_processing_location', 'global', 'Global', 'Global', 'warning', 30
  UNION ALL SELECT 'virtualization', 'data_processing_location', 'local_device', 'Local device', 'Lokales Gerät', 'positive', 40
  ...
  UNION ALL SELECT 'virtualization', 'source_model', 'fully_open_source', 'Fully open source', 'Vollständig quelloffen', 'positive', 10
  UNION ALL SELECT 'virtualization', 'source_model', 'source_available', 'Source available', 'Quellcode verfügbar', 'neutral', 20
  UNION ALL SELECT 'virtualization', 'source_model', 'partially_open', 'Partially open', 'Teilweise offen', 'neutral', 30
  UNION ALL SELECT 'virtualization', 'source_model', 'proprietary', 'Proprietary', 'Proprietär', 'warning', 40
  ...
  UNION ALL SELECT 'virtualization', 'license_type', 'proprietary', 'Proprietary', 'Proprietär', 'neutral', 50
