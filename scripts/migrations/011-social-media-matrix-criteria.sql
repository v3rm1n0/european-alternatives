...
VALUES
  ('social-media', 'identity_profiles', 'Identity & Profiles', 'Identitaet & Profile', 'Account identity, sign-up, pseudonymity, and profile verification controls.', 'Kontoidentitaet, Registrierung, Pseudonymitaet und Kontrollen zur Profilverifikation.', 100),
  ...
  ('social-media', 'content_publishing', 'Content & Publishing', 'Inhalte & Veroeffentlichung', 'Supported post types, media formats, stories, live content, and interaction primitives.', 'Unterstuetzte Beitragstypen, Medienformate, Storys, Live-Inhalte und Interaktionsfunktionen.', 300),
  ...
  ('social-media', 'privacy_safety', 'Privacy & Safety', 'Privatsphaere & Sicherheit', 'Profile visibility, audience controls, direct messages, blocking, and sensitive-content controls.', 'Profilsichtbarkeit, Zielgruppen-Kontrollen, Direktnachrichten, Blockieren und Kontrollen fuer sensible Inhalte.', 500),
  ...
  ('social-media', 'interoperability_portability', 'Interoperability & Portability', 'Interoperabilitaet & Portabilitaet', 'Federation, protocol support, export, account migration, and portability behavior.', 'Foederation, Protokollunterstuetzung, Export, Kontomigration und Portabilitaetsverhalten.', 700),
  ...
  ('social-media', 'access_fit', 'Access & Fit', 'Zugriff & Eignung', 'Platform support, APIs, self-hosting, accessibility, and common user-fit profiles.', 'Plattformunterstuetzung, APIs, Selbsthosting, Barrierefreiheit und typische Eignungsprofile.', 900);
...
FROM (
  SELECT 'social-media' AS category_id, 'identity_profiles' AS group_key, 'real_name_required' AS criterion_key, 'Real name required' AS label_en, 'Klarnamen erforderlich' AS label_de, 'boolean' AS value_type, 'risk' AS semantics, 'must_match' AS filter_mode, 1010 AS sort_order, 'Whether ordinary account use requires a real legal name or comparable identity disclosure.' AS help_text_en, 'Ob die normale Kontonutzung einen echten rechtlichen Namen oder vergleichbare Identitaetsoffenlegung erfordert.' AS help_text_de
  UNION ALL SELECT 'social-media', 'identity_profiles', 'account_signup_methods', 'Account sign-up methods', 'Registrierungsmethoden', 'multi_enum', 'tradeoff', 'multi_select', 1020, 'Documented methods users can use to create or join an account.', 'Dokumentierte Methoden, mit denen Nutzer ein Konto erstellen oder beitreten koennen.'
  UNION ALL SELECT 'social-media', 'identity_profiles', 'pseudonymous_accounts', 'Pseudonymous accounts supported', 'Pseudonyme Konten unterstuetzt', 'boolean', 'beneficial', 'optional', 1030, 'Whether users can operate accounts under handles or pseudonyms without public real-name display.', 'Ob Nutzer Konten unter Handles oder Pseudonymen ohne oeffentliche Klarnamenanzeige betreiben koennen.'
  UNION ALL SELECT 'social-media', 'identity_profiles', 'profile_verification_model', 'Profile verification model', 'Profilverifikation', 'enum', 'informational', 'optional', 1040, 'How profiles, people, organizations, domains, or identities can be verified or badged.', 'Wie Profile, Personen, Organisationen, Domains oder Identitaeten verifiziert oder markiert werden koennen.'
  UNION ALL SELECT 'social-media', 'graph_discovery', 'social_graph_model', 'Social graph model', 'Modell des sozialen Graphen', 'enum', 'tradeoff', 'optional', 2010, 'The main graph structure the service uses for accounts, communities, topics, or professional contacts.', 'Die wichtigste Graph-Struktur, die der Dienst fuer Konten, Communities, Themen oder berufliche Kontakte nutzt.'
  ...
  UNION ALL SELECT 'social-media', 'graph_discovery', 'discovery_methods', 'Discovery methods', 'Auffindbarkeitsmethoden', 'multi_enum', 'informational', 'multi_select', 2030, 'Documented ways users can find accounts, communities, contacts, or topics.', 'Dokumentierte Wege, wie Nutzer Konten, Communities, Kontakte oder Themen finden koennen.'
  UNION ALL SELECT 'social-media', 'graph_discovery', 'hashtags_mentions_support', 'Hashtags and mentions', 'Hashtags und Erwaehnungen', 'multi_enum', 'beneficial', 'multi_select', 2040, 'Discovery and reference primitives such as hashtags, mentions, topics, lists, or trends.', 'Auffindbarkeits- und Referenzfunktionen wie Hashtags, Erwaehnungen, Themen, Listen oder Trends.'
  UNION ALL SELECT 'social-media', 'content_publishing', 'post_content_types', 'Post content types', 'Beitragstypen', 'multi_enum', 'informational', 'multi_select', 3010, 'Types of posts the service supports for normal publishing.', 'Arten von Beitraegen, die der Dienst fuer normale Veroeffentlichungen unterstuetzt.'
  UNION ALL SELECT 'social-media', 'content_publishing', 'max_text_post_length', 'Maximum text post length', 'Maximale Textlaenge', 'number', 'informational', 'range', 3020, 'Documented maximum number of characters or comparable text length for a normal text post.', 'Dokumentierte maximale Zeichenanzahl oder vergleichbare Textlaenge fuer einen normalen Textbeitrag.'
  UNION ALL SELECT 'social-media', 'content_publishing', 'media_upload_types', 'Media upload types', 'Medien-Uploads', 'multi_enum', 'informational', 'multi_select', 3030, 'Media formats users can upload or attach to posts.', 'Medienformate, die Nutzer hochladen oder Beitraegen anhaengen koennen.'
  UNION ALL SELECT 'social-media', 'content_publishing', 'ephemeral_or_story_posts', 'Stories or ephemeral posts', 'Storys oder temporaere Beitraege', 'boolean', 'informational', 'optional', 3040, 'Whether the service supports time-limited stories or other ephemeral post formats.', 'Ob der Dienst zeitlich begrenzte Storys oder andere temporaere Beitragsformate unterstuetzt.'
  UNION ALL SELECT 'social-media', 'content_publishing', 'live_streaming_or_events', 'Live streaming or events', 'Livestreaming oder Events', 'multi_enum', 'informational', 'multi_select', 3050, 'Live, scheduled, or event-based publishing features documented for users or creators.', 'Live-, Planungs- oder Event-Funktionen, die fuer Nutzer oder Creator dokumentiert sind.'
  UNION ALL SELECT 'social-media', 'content_publishing', 'interaction_features', 'Interaction features', 'Interaktionsfunktionen', 'multi_enum', 'informational', 'multi_select', 3060, 'Supported interaction primitives such as likes, reposts, quotes, replies, reactions, or bookmarks.', 'Unterstuetzte Interaktionsfunktionen wie Likes, Reposts, Zitate, Antworten, Reaktionen oder Lesezeichen.'
  UNION ALL SELECT 'social-media', 'feeds_algorithms', 'default_feed_model', 'Default feed model', 'Standard-Feed-Modell', 'enum', 'tradeoff', 'optional', 4010, 'The default feed ordering or curation model users encounter in normal use.', 'Das standardmaessige Feed-Sortierungs- oder Kurationsmodell, das Nutzer im normalen Gebrauch sehen.'
  UNION ALL SELECT 'social-media', 'feeds_algorithms', 'chronological_feed_available', 'Chronological feed available', 'Chronologischer Feed verfuegbar', 'boolean', 'beneficial', 'optional', 4020, 'Whether users can view posts in a chronological or reverse-chronological feed.', 'Ob Nutzer Beitraege in einem chronologischen oder umgekehrt chronologischen Feed sehen koennen.'
  UNION ALL SELECT 'social-media', 'feeds_algorithms', 'feed_algorithm_controls', 'Feed algorithm controls', 'Feed-/Algorithmus-Kontrollen', 'multi_enum', 'beneficial', 'multi_select', 4030, 'User controls for feed ranking, custom feeds, keyword mutes, repost visibility, topics, or recommendations.', 'Nutzerkontrollen fuer Feed-Ranking, eigene Feeds, Stichwort-Stummschaltung, Repost-Sichtbarkeit, Themen oder Empfehlungen.'
  UNION ALL SELECT 'social-media', 'feeds_algorithms', 'recommendation_controls', 'Recommendation controls', 'Empfehlungskontrollen', 'multi_enum', 'beneficial', 'multi_select', 4040, 'Controls for suggested accounts, interests, trending content, or sensitive recommendations.', 'Kontrollen fuer Kontovorschlaege, Interessen, Trends oder sensible Empfehlungen.'
  UNION ALL SELECT 'social-media', 'privacy_safety', 'account_visibility_controls', 'Account visibility controls', 'Kontosichtbarkeit', 'multi_enum', 'beneficial', 'multi_select', 5010, 'Controls for public, private, unlisted, approved-follower, or search-indexed profiles.', 'Kontrollen fuer oeffentliche, private, ungelistete, freigegebene oder suchindexierte Profile.'
  UNION ALL SELECT 'social-media', 'privacy_safety', 'post_audience_controls', 'Post audience controls', 'Zielgruppen fuer Beitraege', 'multi_enum', 'beneficial', 'multi_select', 5020, 'Audience controls users can apply to posts, such as public, followers-only, mentioned users, local instance, or circles.', 'Zielgruppenkontrollen fuer Beitraege, etwa oeffentlich, nur Follower, erwaehnte Nutzer, lokale Instanz oder Kreise.'
  UNION ALL SELECT 'social-media', 'privacy_safety', 'direct_message_model', 'Direct message model', 'Direktnachrichten-Modell', 'enum', 'tradeoff', 'optional', 5030, 'How private or direct messaging works, including absent, restricted, open, or encrypted modes.', 'Wie private oder direkte Nachrichten funktionieren, einschliesslich fehlend, eingeschraenkt, offen oder verschluesselt.'
  UNION ALL SELECT 'social-media', 'privacy_safety', 'blocking_mute_controls', 'Blocking and mute controls', 'Blockier- und Stummschaltkontrollen', 'multi_enum', 'beneficial', 'multi_select', 5040, 'User safety controls for blocking, muting, keyword filtering, domain or instance blocks, hidden replies, or account restrictions.', 'Nutzer-Sicherheitskontrollen fuer Blockieren, Stummschalten, Stichwortfilter, Domain- oder Instanzblocks, ausgeblendete Antworten oder Kontoeinschraenkungen.'
  UNION ALL SELECT 'social-media', 'privacy_safety', 'sensitive_content_controls', 'Sensitive-content controls', 'Kontrollen fuer sensible Inhalte', 'multi_enum', 'beneficial', 'multi_select', 5050, 'Controls for content warnings, sensitive labels, media blur, age gates, filters, or parental settings.', 'Kontrollen fuer Inhaltswarnungen, Sensibel-Labels, Medienweichzeichnung, Altersabfragen, Filter oder Jugendschutzeinstellungen.'
  UNION ALL SELECT 'social-media', 'moderation_governance', 'moderation_model', 'Moderation model', 'Moderationsmodell', 'enum', 'tradeoff', 'optional', 6010, 'The primary moderation model for the service, instance, community, users, or hybrid governance.', 'Das primaere Moderationsmodell fuer Dienst, Instanz, Community, Nutzer oder hybride Governance.'
  UNION ALL SELECT 'social-media', 'moderation_governance', 'community_admin_tools', 'Community admin tools', 'Community-Verwaltungswerkzeuge', 'multi_enum', 'beneficial', 'multi_select', 6020, 'Administration tools for groups or communities, including roles, bans, queues, pinned posts, rules, or logs.', 'Verwaltungswerkzeuge fuer Gruppen oder Communities, einschliesslich Rollen, Sperren, Warteschlangen, angepinnter Beitraege, Regeln oder Protokolle.'
  UNION ALL SELECT 'social-media', 'moderation_governance', 'reporting_enforcement_tools', 'Reporting and enforcement tools', 'Melde- und Durchsetzungswerkzeuge', 'multi_enum', 'beneficial', 'multi_select', 6030, 'User reporting and enforcement paths for spam, abuse, copyright, or trusted flagger workflows.', 'Melde- und Durchsetzungswege fuer Spam, Missbrauch, Urheberrecht oder vertrauenswuerdige Melder.'
  UNION ALL SELECT 'social-media', 'moderation_governance', 'moderation_transparency', 'Moderation transparency', 'Moderationstransparenz', 'enum', 'informational', 'optional', 6040, 'How moderation rules, reports, action notices, or moderation logs are documented or exposed.', 'Wie Moderationsregeln, Berichte, Massnahmenhinweise oder Moderationsprotokolle dokumentiert oder sichtbar gemacht werden.'
  UNION ALL SELECT 'social-media', 'moderation_governance', 'appeal_process_documented', 'Appeal process documented', 'Einspruchsverfahren dokumentiert', 'boolean', 'beneficial', 'optional', 6050, 'Whether users can find a documented process for appealing moderation decisions.', 'Ob Nutzer ein dokumentiertes Verfahren zum Einspruch gegen Moderationsentscheidungen finden koennen.'
  UNION ALL SELECT 'social-media', 'interoperability_portability', 'network_architecture', 'Network architecture', 'Netzwerkarchitektur', 'enum', 'tradeoff', 'optional', 7010, 'Whether the network is centralized, federated, protocol-based, peer-to-peer, self-hosted, or another architecture.', 'Ob das Netzwerk zentralisiert, foederiert, protokollbasiert, Peer-to-peer, selbstgehostet oder anders aufgebaut ist.'
  UNION ALL SELECT 'social-media', 'interoperability_portability', 'federation_protocols', 'Federation protocols', 'Foederationsprotokolle', 'multi_enum', 'tradeoff', 'multi_select', 7020, 'Federation, syndication, or bridge protocols supported by the product or network.', 'Foederations-, Syndikations- oder Bridge-Protokolle, die Produkt oder Netzwerk unterstuetzen.'
  UNION ALL SELECT 'social-media', 'interoperability_portability', 'data_export_available', 'Data export available', 'Datenexport verfuegbar', 'boolean', 'beneficial', 'optional', 7030, 'Whether users can export account data or content through a documented process.', 'Ob Nutzer Kontodaten oder Inhalte ueber einen dokumentierten Prozess exportieren koennen.'
  UNION ALL SELECT 'social-media', 'interoperability_portability', 'account_migration_supported', 'Account migration support', 'Kontomigration unterstuetzt', 'enum', 'tradeoff', 'optional', 7040, 'How account, profile, handle, follower, or full-account migration is supported.', 'Wie Konto-, Profil-, Handle-, Follower- oder vollstaendige Kontomigration unterstuetzt wird.'
  UNION ALL SELECT 'social-media', 'monetization_creator', 'ads_model', 'Ads model', 'Anzeigenmodell', 'enum', 'tradeoff', 'optional', 8010, 'The documented model for ads, promoted posts, personalized ads, creator sponsorships, or no ads.', 'Das dokumentierte Modell fuer Anzeigen, gesponserte Beitraege, personalisierte Anzeigen, Creator-Sponsoring oder keine Anzeigen.'
  UNION ALL SELECT 'social-media', 'monetization_creator', 'ad_personalization_controls', 'Ad personalization controls', 'Kontrollen fuer Anzeigenpersonalisierung', 'multi_enum', 'beneficial', 'multi_select', 8020, 'User controls for personalized ads, interests, tracking, ad topics, paid ad-free tiers, or no-ads states.', 'Nutzerkontrollen fuer personalisierte Anzeigen, Interessen, Tracking, Anzeigenthemen, bezahlte anzeigenfreie Tarife oder Keine-Anzeigen-Zustaende.'
  UNION ALL SELECT 'social-media', 'monetization_creator', 'creator_monetization_tools', 'Creator monetization tools', 'Creator-Monetarisierung', 'multi_enum', 'informational', 'multi_select', 8030, 'Creator income features such as tips, subscriptions, paid posts, revenue share, paid events, or marketplaces.', 'Creator-Einnahmefunktionen wie Trinkgelder, Abos, bezahlte Beitraege, Umsatzbeteiligung, bezahlte Events oder Marktplaetze.'
  UNION ALL SELECT 'social-media', 'monetization_creator', 'commerce_fundraising_tools', 'Commerce and fundraising tools', 'Commerce- und Fundraising-Werkzeuge', 'multi_enum', 'informational', 'multi_select', 8040, 'Commerce, product, fundraising, ticketing, or lead-generation tools documented for accounts or creators.', 'Commerce-, Produkt-, Spenden-, Ticket- oder Lead-Generierungswerkzeuge, die fuer Konten oder Creator dokumentiert sind.'
  UNION ALL SELECT 'social-media', 'access_fit', 'supported_platforms', 'Supported platforms', 'Unterstuetzte Plattformen', 'multi_enum', 'informational', 'multi_select', 9010, 'Maintained platforms, operating systems, stores, or access paths for ordinary users.', 'Gepflegte Plattformen, Betriebssysteme, Stores oder Zugriffswege fuer normale Nutzer.'
  UNION ALL SELECT 'social-media', 'access_fit', 'developer_api', 'Developer API', 'Entwickler-API', 'enum', 'tradeoff', 'optional', 9020, 'The documented availability and model of APIs for developers, integrations, or automation.', 'Die dokumentierte Verfuegbarkeit und das Modell von APIs fuer Entwickler, Integrationen oder Automatisierung.'
  UNION ALL SELECT 'social-media', 'access_fit', 'self_hosting_available', 'Self-hosting available', 'Selbsthosting verfuegbar', 'boolean', 'tradeoff', 'optional', 9030, 'Whether users or organizations can operate compatible server infrastructure themselves.', 'Ob Nutzer oder Organisationen kompatible Server-Infrastruktur selbst betreiben koennen.'
  UNION ALL SELECT 'social-media', 'access_fit', 'accessibility_features', 'Accessibility features', 'Barrierefreiheitsfunktionen', 'multi_enum', 'beneficial', 'multi_select', 9040, 'Documented accessibility support such as alt text, captions, keyboard navigation, screen-reader labels, reduced motion, or translation.', 'Dokumentierte Barrierefreiheitsunterstuetzung wie Alternativtext, Untertitel, Tastaturnavigation, Screenreader-Labels, reduzierte Bewegung oder Uebersetzung.'
  UNION ALL SELECT 'social-media', 'access_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9050, 'Common user needs or audience profiles the social media product is especially suited to support.', 'Typische Nutzerbeduerfnisse oder Zielgruppenprofile, fuer die das Social-Media-Produkt besonders geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'social-media', 'account_signup_methods', 'cryptographic_key', 'Cryptographic key', 'Kryptografischer Schluessel', 'tradeoff', 60
  UNION ALL SELECT 'social-media', 'profile_verification_model', 'none', 'None', 'Keine', 'neutral', 10
  UNION ALL SELECT 'social-media', 'profile_verification_model', 'platform_badge', 'Platform-reviewed badge', 'Plattformgeprueftes Badge', 'neutral', 20
  ...
  UNION ALL SELECT 'social-media', 'profile_verification_model', 'decentralized_identity', 'Decentralized identity', 'Dezentrale Identitaet', 'tradeoff', 60
  ...
  UNION ALL SELECT 'social-media', 'discovery_methods', 'public_directory', 'Public directory', 'Oeffentliches Verzeichnis', 'tradeoff', 30
  ...
  UNION ALL SELECT 'social-media', 'hashtags_mentions_support', 'mentions', 'Mentions', 'Erwaehnungen', 'positive', 20
  ...
  UNION ALL SELECT 'social-media', 'post_content_types', 'short_posts', 'Short posts', 'Kurzbeitraege', 'neutral', 10
  UNION ALL SELECT 'social-media', 'post_content_types', 'long_posts', 'Long posts', 'Lange Beitraege', 'neutral', 20
  ...
  UNION ALL SELECT 'social-media', 'post_content_types', 'link_posts', 'Link posts', 'Link-Beitraege', 'neutral', 50
  ...
  UNION ALL SELECT 'social-media', 'live_streaming_or_events', 'audio_spaces', 'Audio spaces', 'Audio-Raeume', 'neutral', 30
  ...
  UNION ALL SELECT 'social-media', 'interaction_features', 'quote_posts', 'Quote posts', 'Zitat-Beitraege', 'neutral', 30
  ...
  UNION ALL SELECT 'social-media', 'feed_algorithm_controls', 'mute_keywords', 'Mute keywords', 'Stichwoerter stummschalten', 'positive', 30
  ...
  UNION ALL SELECT 'social-media', 'recommendation_controls', 'opt_out_recommendations', 'Recommendation opt-out', 'Opt-out fuer Empfehlungen', 'positive', 10
  ...
  UNION ALL SELECT 'social-media', 'recommendation_controls', 'follow_suggestions', 'Follow suggestions', 'Folgevorschlaege', 'tradeoff', 30
  UNION ALL SELECT 'social-media', 'recommendation_controls', 'trending_controls', 'Trending controls', 'Trend-Kontrollen', 'tradeoff', 40
  UNION ALL SELECT 'social-media', 'recommendation_controls', 'sensitive_recommendation_filters', 'Sensitive recommendation filters', 'Filter fuer sensible Empfehlungen', 'positive', 50
  UNION ALL SELECT 'social-media', 'account_visibility_controls', 'public_profile', 'Public profile', 'Oeffentliches Profil', 'neutral', 10
  ...
  UNION ALL SELECT 'social-media', 'post_audience_controls', 'public_posts', 'Public posts', 'Oeffentliche Beitraege', 'neutral', 10
  UNION ALL SELECT 'social-media', 'post_audience_controls', 'followers_only', 'Followers only', 'Nur Follower', 'positive', 20
  UNION ALL SELECT 'social-media', 'post_audience_controls', 'mentioned_only', 'Mentioned users only', 'Nur erwaehnte Nutzer', 'positive', 30
  ...
  UNION ALL SELECT 'social-media', 'direct_message_model', 'encrypted_dm', 'Encrypted direct messages', 'Verschluesselte Direktnachrichten', 'positive', 50
  ...
  UNION ALL SELECT 'social-media', 'blocking_mute_controls', 'mute_keywords', 'Mute keywords', 'Stichwoerter stummschalten', 'positive', 30
  ...
  UNION ALL SELECT 'social-media', 'blocking_mute_controls', 'restrict_accounts', 'Restrict accounts', 'Konten einschraenken', 'positive', 60
  ...
  UNION ALL SELECT 'social-media', 'moderation_model', 'federated_instance', 'Federated instance', 'Foederierte Instanz', 'tradeoff', 20
  ...
  UNION ALL SELECT 'social-media', 'community_admin_tools', 'pinned_posts', 'Pinned posts', 'Angepinnte Beitraege', 'neutral', 40
  UNION ALL SELECT 'social-media', 'community_admin_tools', 'rule_posts', 'Published rules', 'Veroeffentlichte Regeln', 'positive', 50
  ...
  UNION ALL SELECT 'social-media', 'reporting_enforcement_tools', 'abuse_reports', 'Abuse/harassment reports', 'Missbrauchs-/Belaestigungsmeldungen', 'positive', 30
  ...
  UNION ALL SELECT 'social-media', 'reporting_enforcement_tools', 'trusted_flaggers', 'Trusted flaggers', 'Vertrauenswuerdige Melder', 'tradeoff', 50
  UNION ALL SELECT 'social-media', 'moderation_transparency', 'none', 'None', 'Keine', 'warning', 10
  UNION ALL SELECT 'social-media', 'moderation_transparency', 'public_rules_only', 'Public rules only', 'Nur oeffentliche Regeln', 'neutral', 20
  ...
  UNION ALL SELECT 'social-media', 'moderation_transparency', 'per_action_notices', 'Per-action notices', 'Hinweise je Massnahme', 'positive', 40
  UNION ALL SELECT 'social-media', 'moderation_transparency', 'public_modlog', 'Public moderation log', 'Oeffentliches Moderationsprotokoll', 'tradeoff', 50
  ...
  UNION ALL SELECT 'social-media', 'network_architecture', 'federated', 'Federated', 'Foederiert', 'positive', 20
  ...
  UNION ALL SELECT 'social-media', 'account_migration_supported', 'unsupported', 'Unsupported', 'Nicht unterstuetzt', 'warning', 10
  ...
  UNION ALL SELECT 'social-media', 'account_migration_supported', 'full_account_move', 'Full account move', 'Vollstaendiger Kontoumzug', 'positive', 50
  ...
  UNION ALL SELECT 'social-media', 'ads_model', 'promoted_posts', 'Promoted posts', 'Gesponserte Beitraege', 'tradeoff', 40
  ...
  UNION ALL SELECT 'social-media', 'ad_personalization_controls', 'opt_out_personalized_ads', 'Personalized-ads opt-out', 'Opt-out fuer personalisierte Anzeigen', 'positive', 10
  ...
  UNION ALL SELECT 'social-media', 'ad_personalization_controls', 'third_party_tracking_controls', 'Third-party tracking controls', 'Kontrollen fuer Drittanbieter-Tracking', 'positive', 30
  ...
  UNION ALL SELECT 'social-media', 'creator_monetization_tools', 'paid_posts', 'Paid posts', 'Bezahlte Beitraege', 'tradeoff', 30
  ...
  UNION ALL SELECT 'social-media', 'developer_api', 'public_write_api', 'Public write API', 'Oeffentliche Schreib-API', 'positive', 30
  ...
  UNION ALL SELECT 'social-media', 'accessibility_features', 'language_translation', 'Language translation', 'Sprachuebersetzung', 'neutral', 60
  ...
  UNION ALL SELECT 'social-media', 'fit_profiles', 'journalists_public_figures', 'Journalists/public figures', 'Journalisten/oeffentliche Personen', 'tradeoff', 30
  ...
  UNION ALL SELECT 'social-media', 'fit_profiles', 'nonprofits_campaigns', 'Nonprofits/campaigns', 'Gemeinnuetzige/Kampagnen', 'tradeoff', 60
