...
VALUES
  ('ai-ml', 'model_capabilities', 'Model Capabilities', 'Modellfaehigkeiten', 'What the model can do: context window, modalities, languages, reasoning, and code generation.', 'Was das Modell kann: Kontextfenster, Modalitaeten, Sprachen, logisches Denken und Code-Generierung.', 100),
  ('ai-ml', 'privacy_data', 'Privacy & Data Handling', 'Datenschutz & Datenverarbeitung', 'How user data and conversations are handled, stored, and protected.', 'Wie Nutzerdaten und Unterhaltungen verarbeitet, gespeichert und geschuetzt werden.', 200),
  ('ai-ml', 'pricing_access', 'Pricing & Access', 'Preise & Zugang', 'Pricing models, free tiers, and API availability.', 'Preismodelle, kostenlose Stufen und API-Verfuegbarkeit.', 300),
  ('ai-ml', 'integration_ecosystem', 'Integration & Ecosystem', 'Integration & Oekosystem', 'Platform support, plugins, third-party integrations, and API compatibility.', 'Plattformunterstuetzung, Plugins, Drittanbieter-Integrationen und API-Kompatibilitaet.', 400),
  ...
  ('ai-ml', 'performance_reliability', 'Performance & Reliability', 'Leistung & Zuverlaessigkeit', 'Response speed, uptime guarantees, and streaming support.', 'Antwortgeschwindigkeit, Verfuegbarkeitsgarantien und Streaming-Unterstuetzung.', 600),
  ('ai-ml', 'openness_audit', 'Openness & Audit', 'Offenheit & Audit', 'Model weights availability, security audits, licensing, and model cards.', 'Verfuegbarkeit der Modellgewichte, Sicherheitsaudits, Lizenzierung und Modellkarten.', 700),
  ('ai-ml', 'fit_use_cases', 'Fit & Use Cases', 'Eignung & Anwendungsfaelle', 'Which user profiles the model is best suited for.', 'Fuer welche Nutzerprofile das Modell am besten geeignet ist.', 800);
...
FROM (
  SELECT 'ai-ml' AS category_id, 'model_capabilities' AS group_key, 'context_window_size' AS criterion_key, 'Context window size' AS label_en, 'Kontextfenstergroesse' AS label_de, 'enum' AS value_type, 'informational' AS semantics, 'optional' AS filter_mode, 1010 AS sort_order, 'Maximum number of tokens the model can process in a single request.' AS help_text_en, 'Maximale Anzahl an Tokens, die das Modell in einer einzelnen Anfrage verarbeiten kann.' AS help_text_de
  UNION ALL SELECT 'ai-ml', 'model_capabilities', 'supported_modalities', 'Supported modalities', 'Unterstuetzte Modalitaeten', 'multi_enum', 'informational', 'multi_select', 1020, 'Input and output types supported by the model.', 'Vom Modell unterstuetzte Ein- und Ausgabetypen.'
  UNION ALL SELECT 'ai-ml', 'model_capabilities', 'multilingual_support', 'Multilingual support', 'Mehrsprachige Unterstuetzung', 'enum', 'informational', 'optional', 1030, 'Breadth and depth of language support beyond English.', 'Breite und Tiefe der Sprachunterstuetzung ueber Englisch hinaus.'
  UNION ALL SELECT 'ai-ml', 'model_capabilities', 'reasoning_capability', 'Reasoning capability', 'Schlussfolgerungsfaehigkeit', 'boolean', 'informational', 'optional', 1040, 'Whether the model supports advanced reasoning or chain-of-thought.', 'Ob das Modell erweitertes logisches Denken oder Gedankenketten unterstuetzt.'
  ...
  UNION ALL SELECT 'ai-ml', 'model_capabilities', 'fine_tuning_support', 'Fine-tuning support', 'Feinabstimmungs-Unterstuetzung', 'boolean', 'informational', 'optional', 1060, 'Whether users can fine-tune the model on custom data.', 'Ob Nutzer das Modell mit eigenen Daten feinabstimmen koennen.'
  UNION ALL SELECT 'ai-ml', 'privacy_data', 'training_data_transparency', 'Training data transparency', 'Trainingsdaten-Transparenz', 'enum', 'tradeoff', 'optional', 2010, 'How transparent the provider is about training data sources.', 'Wie transparent der Anbieter bezueglich der Trainingsdatenquellen ist.'
  ...
  UNION ALL SELECT 'ai-ml', 'pricing_access', 'free_tier_available', 'Free tier available', 'Kostenlose Stufe verfuegbar', 'boolean', 'informational', 'optional', 3020, 'Whether a free tier is available without payment.', 'Ob eine kostenlose Nutzungsstufe ohne Zahlung verfuegbar ist.'
  UNION ALL SELECT 'ai-ml', 'pricing_access', 'api_access', 'API access', 'API-Zugang', 'boolean', 'beneficial', 'optional', 3030, 'Whether a developer API is available for programmatic access.', 'Ob eine Entwickler-API fuer programmatischen Zugang verfuegbar ist.'
  ...
  UNION ALL SELECT 'ai-ml', 'integration_ecosystem', 'plugin_ecosystem', 'Plugin or extension ecosystem', 'Plugin- oder Erweiterungs-Oekosystem', 'boolean', 'informational', 'optional', 4010, 'Whether a plugin or extension ecosystem exists.', 'Ob ein Plugin- oder Erweiterungs-Oekosystem existiert.'
  UNION ALL SELECT 'ai-ml', 'integration_ecosystem', 'supported_platforms', 'Supported platforms', 'Unterstuetzte Plattformen', 'multi_enum', 'informational', 'multi_select', 4020, 'Which platforms the model is available on.', 'Auf welchen Plattformen das Modell verfuegbar ist.'
  UNION ALL SELECT 'ai-ml', 'integration_ecosystem', 'third_party_integrations', 'Third-party integrations', 'Drittanbieter-Integrationen', 'boolean', 'informational', 'optional', 4030, 'Whether integrations with third-party tools and services are available.', 'Ob Integrationen mit Drittanbieter-Werkzeugen und -Diensten verfuegbar sind.'
  UNION ALL SELECT 'ai-ml', 'integration_ecosystem', 'api_standard_compatibility', 'API standard compatibility', 'API-Standard-Kompatibilitaet', 'boolean', 'informational', 'optional', 4040, 'Whether the API follows common standards like OpenAI-compatible endpoints.', 'Ob die API gaengige Standards wie OpenAI-kompatible Endpunkte befolgt.'
  ...
  UNION ALL SELECT 'ai-ml', 'safety_alignment', 'safety_documentation', 'Safety documentation', 'Sicherheitsdokumentation', 'boolean', 'beneficial', 'optional', 5020, 'Whether safety practices and policies are publicly documented.', 'Ob Sicherheitspraktiken und -richtlinien oeffentlich dokumentiert sind.'
  UNION ALL SELECT 'ai-ml', 'safety_alignment', 'output_attribution', 'Output attribution', 'Ausgabezuordnung', 'boolean', 'informational', 'optional', 5030, 'Whether the model provides source attribution for generated content.', 'Ob das Modell Quellenangaben fuer generierte Inhalte bereitstellt.'
  UNION ALL SELECT 'ai-ml', 'performance_reliability', 'response_speed', 'Response speed', 'Antwortgeschwindigkeit', 'enum', 'informational', 'optional', 6010, 'Typical response speed for standard queries.', 'Typische Antwortgeschwindigkeit fuer Standardanfragen.'
  UNION ALL SELECT 'ai-ml', 'performance_reliability', 'uptime_sla', 'Uptime SLA', 'Verfuegbarkeits-SLA', 'boolean', 'beneficial', 'optional', 6020, 'Whether a formal uptime service level agreement is offered.', 'Ob eine formelle Verfuegbarkeits-Dienstguetevereinbarung angeboten wird.'
  UNION ALL SELECT 'ai-ml', 'performance_reliability', 'streaming_support', 'Streaming support', 'Streaming-Unterstuetzung', 'boolean', 'beneficial', 'optional', 6030, 'Whether streaming responses are supported for real-time output.', 'Ob Streaming-Antworten fuer Echtzeit-Ausgabe unterstuetzt werden.'
  UNION ALL SELECT 'ai-ml', 'openness_audit', 'model_weights_availability', 'Model weights availability', 'Verfuegbarkeit der Modellgewichte', 'enum', 'tradeoff', 'optional', 7010, 'Whether the model weights are openly available for download.', 'Ob die Modellgewichte offen zum Herunterladen verfuegbar sind.'
  UNION ALL SELECT 'ai-ml', 'openness_audit', 'security_audit', 'Independent security audit', 'Unabhaengiges Sicherheitsaudit', 'boolean', 'beneficial', 'optional', 7020, 'Whether an independent security audit has been completed.', 'Ob ein unabhaengiges Sicherheitsaudit durchgefuehrt wurde.'
  ...
  UNION ALL SELECT 'ai-ml', 'openness_audit', 'model_card_published', 'Model card published', 'Modellkarte veroeffentlicht', 'boolean', 'informational', 'optional', 7040, 'Whether a model card with capabilities and limitations is published.', 'Ob eine Modellkarte mit Faehigkeiten und Einschraenkungen veroeffentlicht wurde.'
  UNION ALL SELECT 'ai-ml', 'fit_use_cases', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 8010, 'Common user profiles the model is well suited for.', 'Typische Nutzerprofile, fuer die das Modell gut geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'ai-ml', 'context_window_size', 'above_128k', 'Above 128K tokens', 'Ueber 128K Tokens', 'neutral', 40
  ...
  UNION ALL SELECT 'ai-ml', 'multilingual_support', 'limited', 'Limited', 'Eingeschraenkt', 'neutral', 20
  ...
  UNION ALL SELECT 'ai-ml', 'training_data_transparency', 'full_disclosure', 'Full disclosure', 'Vollstaendige Offenlegung', 'positive', 10
  ...
  UNION ALL SELECT 'ai-ml', 'gdpr_data_processing_location', 'eu_primary', 'EU primary', 'EU primaer', 'neutral', 20
  ...
  UNION ALL SELECT 'ai-ml', 'model_weights_availability', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 30
  ...
  UNION ALL SELECT 'ai-ml', 'license_type', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 50
  ...
  UNION ALL SELECT 'ai-ml', 'fit_profiles', 'enterprise', 'Enterprise', 'Grossunternehmen', 'neutral', 60
) AS d
