...
VALUES
  ('app-stores', 'distribution_platform', 'Distribution & Platform', 'Verteilung & Plattform', 'How the store distributes apps, which platforms are supported, and which installation methods are available.', 'Wie der Store Apps verteilt, welche Plattformen unterstuetzt werden und welche Installationsmethoden verfuegbar sind.', 100),
  ('app-stores', 'app_catalog_curation', 'App Catalog & Curation', 'App-Katalog & Kuratierung', 'App catalog scope, curation approach, review process, and discovery features.', 'Umfang des App-Katalogs, Kuratierungsansatz, Pruefprozess und Entdeckungsfunktionen.', 200),
  ('app-stores', 'privacy_tracking', 'Privacy & Tracking', 'Privatsphaere & Tracking', 'Privacy protections, tracker detection, account requirements, and telemetry behavior of the store.', 'Datenschutzmassnahmen, Tracker-Erkennung, Kontoanforderungen und Telemetrieverhalten des Stores.', 300),
  ('app-stores', 'security_updates', 'Security & Updates', 'Sicherheit & Aktualisierungen', 'Security measures for app verification, update delivery, and build integrity.', 'Sicherheitsmassnahmen fuer App-Verifizierung, Update-Bereitstellung und Build-Integritaet.', 400),
  ('app-stores', 'developer_ecosystem', 'Developer Ecosystem', 'Entwickler-Oekosystem', 'Developer onboarding, fees, revenue models, and documentation quality.', 'Entwickler-Onboarding, Gebuehren, Erloesmodelle und Dokumentationsqualitaet.', 500),
  ...
  ('app-stores', 'openness_transparency', 'Openness & Transparency', 'Offenheit & Transparenz', 'Source code availability, metadata openness, governance, and independent audits.', 'Quellcodeverfuegbarkeit, Metadatenoffenheit, Governance und unabhaengige Pruefungen.', 700),
  ('app-stores', 'data_compliance', 'Data & Compliance', 'Daten & Compliance', 'Data processing location, GDPR compliance, account deletion, and self-hosting options.', 'Datenverarbeitungsstandort, DSGVO-Konformitaet, Kontoloeschung und Self-Hosting-Optionen.', 800),
  ('app-stores', 'pricing_fit', 'Pricing & Fit', 'Preise & Eignung', 'Pricing structure, free tier availability, and target audience fit profiles.', 'Preisstruktur, Verfuegbarkeit kostenloser Stufen und Zielgruppen-Eignungsprofile.', 900);
...
FROM (
  SELECT 'app-stores' AS category_id, 'distribution_platform' AS group_key, 'target_platform' AS criterion_key, 'Target platform' AS label_en, 'Zielplattform' AS label_de, 'multi_enum' AS value_type, 'informational' AS semantics, 'multi_select' AS filter_mode, 1010 AS sort_order, 'Operating systems and platforms the app store natively supports.' AS help_text_en, 'Betriebssysteme und Plattformen, die der App Store nativ unterstuetzt.' AS help_text_de
  UNION ALL SELECT 'app-stores', 'distribution_platform', 'distribution_model', 'Distribution model', 'Verteilungsmodell', 'enum', 'tradeoff', 'optional', 1020, 'How apps are distributed, whether centrally, through repositories, or in a decentralized manner.', 'Wie Apps verteilt werden, ob zentral, ueber Repositories oder dezentral.'
  UNION ALL SELECT 'app-stores', 'distribution_platform', 'installation_method', 'Installation method', 'Installationsmethode', 'multi_enum', 'informational', 'multi_select', 1030, 'Technical methods available for installing apps from the store.', 'Technische Methoden, die fuer die Installation von Apps aus dem Store verfuegbar sind.'
  UNION ALL SELECT 'app-stores', 'distribution_platform', 'sideloading_support', 'Sideloading support', 'Sideloading-Unterstuetzung', 'boolean', 'beneficial', 'optional', 1040, 'Whether the store allows installing apps from sources outside its own catalog.', 'Ob der Store die Installation von Apps aus Quellen ausserhalb des eigenen Katalogs erlaubt.'
  UNION ALL SELECT 'app-stores', 'distribution_platform', 'offline_app_install', 'Offline app installation', 'Offline-App-Installation', 'boolean', 'informational', 'optional', 1050, 'Whether apps can be installed without an active internet connection.', 'Ob Apps ohne aktive Internetverbindung installiert werden koennen.'
  UNION ALL SELECT 'app-stores', 'app_catalog_curation', 'review_process', 'App review process', 'App-Pruefprozess', 'enum', 'tradeoff', 'optional', 2010, 'How submitted apps are reviewed before being listed in the store.', 'Wie eingereichte Apps geprueft werden, bevor sie im Store gelistet werden.'
  UNION ALL SELECT 'app-stores', 'app_catalog_curation', 'foss_focus', 'FOSS app focus', 'FOSS-App-Fokus', 'boolean', 'informational', 'optional', 2020, 'Whether the store primarily focuses on free and open-source software.', 'Ob der Store sich primaer auf freie und quelloffene Software konzentriert.'
  UNION ALL SELECT 'app-stores', 'app_catalog_curation', 'anti_feature_flags', 'Anti-feature flags', 'Anti-Feature-Kennzeichnung', 'boolean', 'beneficial', 'optional', 2030, 'Whether the store labels apps that contain anti-features such as ads, tracking, or non-free dependencies.', 'Ob der Store Apps kennzeichnet, die Anti-Features wie Werbung, Tracking oder unfreie Abhaengigkeiten enthalten.'
  UNION ALL SELECT 'app-stores', 'app_catalog_curation', 'user_ratings_reviews', 'User ratings & reviews', 'Nutzerbewertungen & Rezensionen', 'boolean', 'informational', 'optional', 2040, 'Whether users can rate and review apps within the store.', 'Ob Nutzer Apps innerhalb des Stores bewerten und rezensieren koennen.'
  ...
  UNION ALL SELECT 'app-stores', 'app_catalog_curation', 'app_categories_available', 'App category breadth', 'App-Kategoriebreite', 'enum', 'informational', 'optional', 2060, 'The breadth of app categories available in the store catalog.', 'Die Breite der im Store-Katalog verfuegbaren App-Kategorien.'
  ...
  UNION ALL SELECT 'app-stores', 'security_updates', 'reproducible_builds', 'Reproducible builds support', 'Reproduzierbare-Builds-Unterstuetzung', 'boolean', 'beneficial', 'optional', 4030, 'Whether the store supports reproducible builds to verify binary-to-source correspondence.', 'Ob der Store reproduzierbare Builds unterstuetzt, um die Uebereinstimmung von Binaerdatei und Quellcode zu verifizieren.'
  UNION ALL SELECT 'app-stores', 'security_updates', 'auto_update_mechanism', 'Auto-update mechanism', 'Auto-Update-Mechanismus', 'enum', 'informational', 'optional', 4040, 'How the store handles automatic updates for installed apps.', 'Wie der Store automatische Aktualisierungen fuer installierte Apps handhabt.'
  UNION ALL SELECT 'app-stores', 'security_updates', 'update_notification', 'Update notifications', 'Update-Benachrichtigungen', 'boolean', 'informational', 'optional', 4050, 'Whether the store notifies users when app updates are available.', 'Ob der Store Benutzer benachrichtigt, wenn App-Aktualisierungen verfuegbar sind.'
  UNION ALL SELECT 'app-stores', 'security_updates', 'delta_updates', 'Delta / incremental updates', 'Delta- / inkrementelle Updates', 'boolean', 'informational', 'optional', 4060, 'Whether the store supports delta updates that download only changed portions of an app.', 'Ob der Store Delta-Updates unterstuetzt, die nur geaenderte Teile einer App herunterladen.'
  UNION ALL SELECT 'app-stores', 'developer_ecosystem', 'developer_registration_fee', 'Developer registration fee', 'Entwickler-Registrierungsgebuehr', 'enum', 'informational', 'optional', 5010, 'The fee structure required for developers to register and publish apps.', 'Die Gebuehrenstruktur, die fuer Entwickler zur Registrierung und Veroeffentlichung von Apps erforderlich ist.'
  UNION ALL SELECT 'app-stores', 'developer_ecosystem', 'revenue_share_model', 'Revenue share model', 'Erloesteilungsmodell', 'enum', 'informational', 'optional', 5020, 'How revenue from paid apps or in-app purchases is shared between developer and store.', 'Wie Einnahmen aus kostenpflichtigen Apps oder In-App-Kaeufen zwischen Entwickler und Store aufgeteilt werden.'
  UNION ALL SELECT 'app-stores', 'developer_ecosystem', 'developer_api', 'Developer API available', 'Entwickler-API verfuegbar', 'boolean', 'informational', 'optional', 5030, 'Whether a programmatic API is available for developers to manage listings and metadata.', 'Ob eine programmatische API fuer Entwickler zur Verwaltung von Eintraegen und Metadaten verfuegbar ist.'
  ...
  UNION ALL SELECT 'app-stores', 'developer_ecosystem', 'developer_documentation', 'Developer documentation', 'Entwicklerdokumentation', 'enum', 'informational', 'optional', 5050, 'The quality and completeness of documentation available for developers.', 'Die Qualitaet und Vollstaendigkeit der fuer Entwickler verfuegbaren Dokumentation.'
  UNION ALL SELECT 'app-stores', 'user_experience', 'web_storefront', 'Web storefront', 'Web-Storefront', 'boolean', 'informational', 'optional', 6010, 'Whether apps can be browsed and discovered through a web-based storefront.', 'Ob Apps ueber eine webbasierte Storefront durchsucht und entdeckt werden koennen.'
  ...
  UNION ALL SELECT 'app-stores', 'user_experience', 'multiple_app_sources', 'Multiple repository / source support', 'Mehrere Repositorys / Quellen', 'boolean', 'informational', 'optional', 6040, 'Whether the store supports adding multiple repositories or app sources.', 'Ob der Store das Hinzufuegen mehrerer Repositorys oder App-Quellen unterstuetzt.'
  UNION ALL SELECT 'app-stores', 'user_experience', 'parental_controls', 'Parental controls', 'Kindersicherung', 'boolean', 'informational', 'optional', 6050, 'Whether parental controls or content filtering options are available.', 'Ob Kindersicherung oder Inhaltsfilterungsoptionen verfuegbar sind.'
  ...
  UNION ALL SELECT 'app-stores', 'openness_transparency', 'independent_security_audit', 'Independent security audit', 'Unabhaengiges Sicherheitsaudit', 'boolean', 'beneficial', 'optional', 7040, 'Whether the store has undergone an independent security audit by a third party.', 'Ob der Store ein unabhaengiges Sicherheitsaudit durch einen Dritten durchlaufen hat.'
  ...
  UNION ALL SELECT 'app-stores', 'data_compliance', 'gdpr_dpa_available', 'GDPR DPA available', 'DSGVO-AVV verfuegbar', 'boolean', 'informational', 'optional', 8020, 'Whether a GDPR-compliant data processing agreement is offered to users or developers.', 'Ob ein DSGVO-konformer Auftragsverarbeitungsvertrag fuer Nutzer oder Entwickler angeboten wird.'
  UNION ALL SELECT 'app-stores', 'data_compliance', 'account_deletion', 'Account deletion', 'Kontoloeschung', 'boolean', 'beneficial', 'optional', 8030, 'Whether users can fully delete their accounts and all associated data.', 'Ob Benutzer ihre Konten und alle zugehoerigen Daten vollstaendig loeschen koennen.'
  ...
  UNION ALL SELECT 'app-stores', 'pricing_fit', 'pricing_model', 'Pricing model', 'Preismodell', 'enum', 'informational', 'optional', 9010, 'How the store is priced for end users.', 'Wie der Store fuer Endnutzer bepreist wird.'
  UNION ALL SELECT 'app-stores', 'pricing_fit', 'free_tier_available', 'Free tier available', 'Kostenlose Stufe verfuegbar', 'boolean', 'informational', 'must_match', 9020, 'Whether the store offers a free tier without requiring payment.', 'Ob der Store eine kostenlose Stufe ohne Bezahlung anbietet.'
  UNION ALL SELECT 'app-stores', 'pricing_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9030, 'Target audience profiles the store is best suited for.', 'Zielgruppenprofile, fuer die der Store am besten geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'app-stores', 'distribution_model', 'decentralized_federated', 'Decentralized / federated', 'Dezentralisiert / foederiert', 'positive', 20
  ...
  UNION ALL SELECT 'app-stores', 'review_process', 'manual_review', 'Manual review', 'Manuelle Pruefung', 'positive', 10
  UNION ALL SELECT 'app-stores', 'review_process', 'automated_scan', 'Automated scan', 'Automatisierter Scan', 'neutral', 20
  UNION ALL SELECT 'app-stores', 'review_process', 'community_review', 'Community review', 'Community-Pruefung', 'neutral', 30
  ...
  UNION ALL SELECT 'app-stores', 'auto_update_mechanism', 'user_triggered', 'User-triggered', 'Vom Benutzer ausgeloest', 'neutral', 20
  ...
  UNION ALL SELECT 'app-stores', 'developer_registration_fee', 'annual_fee', 'Annual fee', 'Jahresgebuehr', 'neutral', 40
  ...
  UNION ALL SELECT 'app-stores', 'app_submission_process', 'self_publish', 'Self-publish', 'Selbstveroeffentlichung', 'neutral', 40
  ...
  UNION ALL SELECT 'app-stores', 'source_model', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 40
  UNION ALL SELECT 'app-stores', 'app_metadata_transparency', 'fully_open', 'Fully open', 'Vollstaendig offen', 'positive', 10
  UNION ALL SELECT 'app-stores', 'app_metadata_transparency', 'partially_open', 'Partially open', 'Teilweise offen', 'neutral', 20
  UNION ALL SELECT 'app-stores', 'app_metadata_transparency', 'proprietary', 'Proprietary', 'Proprietaer', 'warning', 30
  UNION ALL SELECT 'app-stores', 'data_processing_location', 'eu_only', 'EU/EEA only', 'Nur EU/EWR', 'positive', 10
  UNION ALL SELECT 'app-stores', 'data_processing_location', 'eu_primary', 'EU primary', 'EU primaer', 'neutral', 20
  ...
  UNION ALL SELECT 'app-stores', 'fit_profiles', 'personal', 'Personal', 'Persoenlich', 'neutral', 10
  ...
  UNION ALL SELECT 'app-stores', 'fit_profiles', 'foss_advocate', 'FOSS advocate', 'FOSS-Befuerworter', 'neutral', 70
) AS d
