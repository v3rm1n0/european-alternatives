...
VALUES
  ...
  ('vpn', 'encryption_protocols', 'Encryption & Protocols', 'Verschlüsselung & Protokolle', 'VPN tunnel protocols, cipher strength, and key exchange mechanisms.', 'VPN-Tunnel-Protokolle, Verschlüsselungsstärke und Schlüsselaustausch-Mechanismen.', 200),
  ('vpn', 'privacy_logging', 'Privacy & Logging Policy', 'Datenschutz & Protokollierungsrichtlinie', 'No-log policies, independent audits, and legal jurisdiction of the provider.', 'No-Log-Richtlinien, unabhängige Audits und rechtliche Zuständigkeit des Anbieters.', 300),
  ('vpn', 'leak_protection', 'Leak Protection & Kill Switch', 'Leckschutz & Kill Switch', 'DNS, IPv6, and WebRTC leak prevention and kill switch functionality.', 'DNS-, IPv6- und WebRTC-Leckschutz sowie Kill-Switch-Funktionalität.', 400),
  ('vpn', 'platform_apps', 'Platform & Apps', 'Plattform & Apps', 'Supported operating systems, router compatibility, and simultaneous connections.', 'Unterstützte Betriebssysteme, Router-Kompatibilität und gleichzeitige Verbindungen.', 500),
  ...
  ('vpn', 'advanced_features', 'Advanced Features', 'Erweiterte Funktionen', 'Split tunneling, multi-hop, Tor-over-VPN, port forwarding, and other power user features.', 'Split Tunneling, Multi-Hop, Tor-über-VPN, Portweiterleitung und weitere Funktionen für fortgeschrittene Nutzer.', 700),
  ('vpn', 'account_payment', 'Account & Payment', 'Konto & Bezahlung', 'Anonymous signup, payment methods, free tiers, and refund guarantees.', 'Anonyme Anmeldung, Zahlungsmethoden, kostenlose Stufen und Rückerstattungsgarantien.', 800),
...
FROM (
  SELECT 'vpn' AS category_id, 'network_infrastructure' AS group_key, 'server_country_count' AS criterion_key, 'Server country count' AS label_en, 'Serverländeranzahl' AS label_de, 'number' AS value_type, 'informational' AS semantics, 'range' AS filter_mode, 1010 AS sort_order, 'Number of countries where the VPN provider operates servers.' AS help_text_en, 'Anzahl der Länder, in denen der VPN-Anbieter Server betreibt.' AS help_text_de
  UNION ALL SELECT 'vpn', 'network_infrastructure', 'server_count_range', 'Server count range', 'Serveranzahl-Bereich', 'enum', 'informational', 'optional', 1020, 'Approximate total number of servers in the provider network.', 'Ungefähre Gesamtzahl der Server im Anbieternetzwerk.'
  ...
  UNION ALL SELECT 'vpn', 'network_infrastructure', 'ram_only_servers', 'RAM-only (diskless) servers', 'Nur-RAM-Server (festplattenlos)', 'boolean', 'beneficial', 'optional', 1040, 'Whether servers run entirely in RAM without persistent storage.', 'Ob Server vollständig im RAM ohne dauerhaften Speicher laufen.'
  ...
  UNION ALL SELECT 'vpn', 'encryption_protocols', 'supported_vpn_protocols', 'Supported VPN protocols', 'Unterstützte VPN-Protokolle', 'multi_enum', 'informational', 'multi_select', 2010, 'VPN tunnel protocols supported by the provider.', 'Vom Anbieter unterstützte VPN-Tunnel-Protokolle.'
  UNION ALL SELECT 'vpn', 'encryption_protocols', 'default_vpn_protocol', 'Default VPN protocol', 'Standard-VPN-Protokoll', 'enum', 'tradeoff', 'optional', 2020, 'The VPN protocol used by default in client applications.', 'Das standardmäßig in Client-Anwendungen verwendete VPN-Protokoll.'
  UNION ALL SELECT 'vpn', 'encryption_protocols', 'cipher_strength', 'Cipher strength', 'Verschlüsselungsstärke', 'enum', 'beneficial', 'must_match', 2030, 'The encryption cipher used for the VPN tunnel.', 'Die für den VPN-Tunnel verwendete Verschlüsselungschiffre.'
  UNION ALL SELECT 'vpn', 'encryption_protocols', 'perfect_forward_secrecy', 'Perfect forward secrecy', 'Perfect Forward Secrecy', 'boolean', 'beneficial', 'optional', 2040, 'Whether the VPN uses ephemeral keys to prevent retroactive decryption.', 'Ob das VPN ephemere Schlüssel verwendet, um rückwirkende Entschlüsselung zu verhindern.'
  UNION ALL SELECT 'vpn', 'encryption_protocols', 'post_quantum_readiness', 'Post-quantum readiness', 'Post-Quanten-Bereitschaft', 'enum', 'informational', 'optional', 2050, 'Readiness for post-quantum cryptographic standards.', 'Bereitschaft für Post-Quanten-Kryptographiestandards.'
  UNION ALL SELECT 'vpn', 'privacy_logging', 'logging_policy', 'Logging policy', 'Protokollierungsrichtlinie', 'enum', 'risk', 'must_match', 3010, 'The type and extent of user activity and connection data logged by the provider.', 'Art und Umfang der vom Anbieter protokollierten Nutzeraktivitäts- und Verbindungsdaten.'
  UNION ALL SELECT 'vpn', 'privacy_logging', 'independent_audit_status', 'Independent audit status', 'Unabhängiger Audit-Status', 'enum', 'beneficial', 'optional', 3020, 'Whether the provider has undergone independent security or no-log audits.', 'Ob der Anbieter unabhängige Sicherheits- oder No-Log-Audits durchlaufen hat.'
  UNION ALL SELECT 'vpn', 'privacy_logging', 'jurisdiction', 'Legal jurisdiction', 'Rechtliche Zuständigkeit', 'enum', 'tradeoff', 'optional', 3030, 'The legal jurisdiction under which the VPN provider operates.', 'Die rechtliche Zuständigkeit, unter der der VPN-Anbieter operiert.'
  UNION ALL SELECT 'vpn', 'privacy_logging', 'warrant_canary', 'Warrant canary', 'Warrant Canary', 'boolean', 'beneficial', 'optional', 3040, 'Whether the provider publishes a warrant canary to signal absence of secret government orders.', 'Ob der Anbieter einen Warrant Canary veröffentlicht, um das Fehlen geheimer Regierungsanordnungen zu signalisieren.'
  UNION ALL SELECT 'vpn', 'privacy_logging', 'transparency_report', 'Transparency report', 'Transparenzbericht', 'boolean', 'beneficial', 'optional', 3050, 'Whether the provider regularly publishes transparency reports on law enforcement requests.', 'Ob der Anbieter regelmäßig Transparenzberichte über Anfragen von Strafverfolgungsbehörden veröffentlicht.'
  UNION ALL SELECT 'vpn', 'leak_protection', 'kill_switch', 'Kill switch', 'Kill Switch', 'enum', 'beneficial', 'must_match', 4010, 'Whether the VPN includes a kill switch to block traffic if the connection drops.', 'Ob das VPN einen Kill Switch zum Blockieren des Datenverkehrs bei Verbindungsabbruch enthält.'
  ...
  UNION ALL SELECT 'vpn', 'leak_protection', 'custom_dns_support', 'Custom DNS support', 'Eigene DNS-Unterstützung', 'boolean', 'beneficial', 'optional', 4050, 'Whether the VPN allows using custom DNS resolvers.', 'Ob das VPN die Nutzung eigener DNS-Resolver erlaubt.'
  UNION ALL SELECT 'vpn', 'platform_apps', 'supported_platforms', 'Supported platforms', 'Unterstützte Plattformen', 'multi_enum', 'informational', 'multi_select', 5010, 'Operating systems and platforms supported by the VPN client.', 'Vom VPN-Client unterstützte Betriebssysteme und Plattformen.'
  UNION ALL SELECT 'vpn', 'platform_apps', 'browser_extension_available', 'Browser extension available', 'Browser-Erweiterung verfügbar', 'boolean', 'beneficial', 'optional', 5020, 'Whether a browser extension is available as a lightweight VPN proxy.', 'Ob eine Browser-Erweiterung als leichtgewichtiger VPN-Proxy verfügbar ist.'
  UNION ALL SELECT 'vpn', 'platform_apps', 'simultaneous_connections', 'Simultaneous connections', 'Gleichzeitige Verbindungen', 'enum', 'informational', 'optional', 5030, 'Maximum number of devices that can connect simultaneously.', 'Maximale Anzahl von Geräten, die gleichzeitig verbunden sein können.'
  UNION ALL SELECT 'vpn', 'platform_apps', 'router_firmware_support', 'Router firmware support', 'Router-Firmware-Unterstützung', 'multi_enum', 'informational', 'multi_select', 5040, 'Router firmware platforms supported for VPN configuration.', 'Unterstützte Router-Firmware-Plattformen für VPN-Konfiguration.'
  UNION ALL SELECT 'vpn', 'platform_apps', 'linux_gui_available', 'Linux GUI available', 'Linux-GUI verfügbar', 'boolean', 'beneficial', 'optional', 5050, 'Whether a graphical user interface is available for the Linux client.', 'Ob eine grafische Benutzeroberfläche für den Linux-Client verfügbar ist.'
  ...
  UNION ALL SELECT 'vpn', 'streaming_performance', 'data_cap', 'Monthly data cap', 'Monatliches Datenlimit', 'enum', 'informational', 'optional', 6020, 'Monthly data transfer allowance for the VPN service.', 'Monatliches Datentransfervolumen für den VPN-Dienst.'
  UNION ALL SELECT 'vpn', 'streaming_performance', 'streaming_unblock_capability', 'Streaming unblock capability', 'Streaming-Entsperrfähigkeit', 'enum', 'informational', 'optional', 6030, 'Ability to reliably access geo-restricted streaming content.', 'Fähigkeit, zuverlässig auf geografisch eingeschränkte Streaming-Inhalte zuzugreifen.'
  ...
  UNION ALL SELECT 'vpn', 'advanced_features', 'split_tunneling', 'Split tunneling', 'Split Tunneling', 'enum', 'beneficial', 'optional', 7010, 'Whether selected traffic can bypass the VPN tunnel.', 'Ob ausgewählter Datenverkehr den VPN-Tunnel umgehen kann.'
  UNION ALL SELECT 'vpn', 'advanced_features', 'multi_hop', 'Multi-hop (double VPN)', 'Multi-Hop (Doppel-VPN)', 'boolean', 'beneficial', 'optional', 7020, 'Whether traffic can be routed through two or more VPN servers.', 'Ob Datenverkehr über zwei oder mehr VPN-Server geleitet werden kann.'
  UNION ALL SELECT 'vpn', 'advanced_features', 'tor_over_vpn', 'Tor-over-VPN', 'Tor-über-VPN', 'boolean', 'informational', 'optional', 7030, 'Whether built-in Tor integration is available for additional anonymity.', 'Ob eine integrierte Tor-Integration für zusätzliche Anonymität verfügbar ist.'
  UNION ALL SELECT 'vpn', 'advanced_features', 'port_forwarding', 'Port forwarding', 'Portweiterleitung', 'boolean', 'informational', 'optional', 7040, 'Whether the VPN supports port forwarding for incoming connections.', 'Ob das VPN Portweiterleitung für eingehende Verbindungen unterstützt.'
  UNION ALL SELECT 'vpn', 'advanced_features', 'ad_tracker_blocker', 'Built-in ad/tracker blocker', 'Integrierter Werbe-/Tracker-Blocker', 'boolean', 'beneficial', 'optional', 7050, 'Whether the VPN includes DNS-level ad and tracker blocking.', 'Ob das VPN DNS-basierte Werbe- und Tracker-Blockierung enthält.'
  UNION ALL SELECT 'vpn', 'advanced_features', 'dedicated_ip_available', 'Dedicated IP available', 'Dedizierte IP verfügbar', 'boolean', 'informational', 'optional', 7060, 'Whether a static dedicated IP address option is available.', 'Ob eine statische dedizierte IP-Adresse als Option verfügbar ist.'
  UNION ALL SELECT 'vpn', 'account_payment', 'anonymous_signup', 'Anonymous signup', 'Anonyme Anmeldung', 'enum', 'beneficial', 'optional', 8010, 'Whether identity information is required to create an account.', 'Ob Identitätsinformationen zur Kontoerstellung erforderlich sind.'
  UNION ALL SELECT 'vpn', 'account_payment', 'cryptocurrency_payment', 'Cryptocurrency payment', 'Kryptowährungszahlung', 'boolean', 'beneficial', 'optional', 8020, 'Whether cryptocurrency is accepted as a payment method.', 'Ob Kryptowährung als Zahlungsmethode akzeptiert wird.'
  UNION ALL SELECT 'vpn', 'account_payment', 'free_tier_available', 'Free tier available', 'Kostenlose Stufe verfügbar', 'boolean', 'informational', 'optional', 8030, 'Whether a meaningful free tier is available without payment.', 'Ob eine sinnvolle kostenlose Stufe ohne Zahlung verfügbar ist.'
  UNION ALL SELECT 'vpn', 'account_payment', 'money_back_guarantee_days', 'Money-back guarantee (days)', 'Geld-zurück-Garantie (Tage)', 'number', 'informational', 'range', 8040, 'Number of days for the money-back guarantee refund window.', 'Anzahl der Tage für das Rückerstattungsfenster der Geld-zurück-Garantie.'
  ...
  UNION ALL SELECT 'vpn', 'openness_governance_fit', 'bug_bounty_program', 'Bug bounty program', 'Bug-Bounty-Programm', 'boolean', 'beneficial', 'optional', 9040, 'Whether the provider runs a security vulnerability reward program.', 'Ob der Anbieter ein Sicherheitslücken-Belohnungsprogramm betreibt.'
  UNION ALL SELECT 'vpn', 'openness_governance_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9050, 'Common user profiles or use cases the VPN service is well suited for.', 'Typische Nutzerprofile oder Anwendungsfälle, für die der VPN-Dienst gut geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'vpn', 'server_count_range', 'over_5000', 'Over 5,000', 'Über 5.000', 'neutral', 50
  UNION ALL SELECT 'vpn', 'owns_physical_servers', 'all_owned', 'All owned', 'Alle im Besitz', 'positive', 10
  UNION ALL SELECT 'vpn', 'owns_physical_servers', 'mostly_owned', 'Mostly owned', 'Überwiegend im Besitz', 'positive', 20
  UNION ALL SELECT 'vpn', 'owns_physical_servers', 'mixed', 'Mixed', 'Gemischt', 'neutral', 30
  UNION ALL SELECT 'vpn', 'owns_physical_servers', 'mostly_rented', 'Mostly rented', 'Überwiegend gemietet', 'tradeoff', 40
  ...
  UNION ALL SELECT 'vpn', 'supported_vpn_protocols', 'proprietary', 'Proprietary', 'Proprietär', 'tradeoff', 40
  ...
  UNION ALL SELECT 'vpn', 'default_vpn_protocol', 'proprietary', 'Proprietary', 'Proprietär', 'tradeoff', 40
  ...
  UNION ALL SELECT 'vpn', 'cipher_strength', 'proprietary_cipher', 'Proprietary cipher', 'Proprietäre Chiffre', 'warning', 40
  ...
  UNION ALL SELECT 'vpn', 'post_quantum_readiness', 'announced', 'Announced', 'Angekündigt', 'neutral', 30
  ...
  UNION ALL SELECT 'vpn', 'logging_policy', 'no_activity_logs', 'No activity logs', 'Keine Aktivitätsprotokolle', 'positive', 20
  ...
  UNION ALL SELECT 'vpn', 'logging_policy', 'full_logs', 'Full logs', 'Vollständige Protokolle', 'negative', 50
  ...
  UNION ALL SELECT 'vpn', 'kill_switch', 'not_available', 'Not available', 'Nicht verfügbar', 'warning', 30
  UNION ALL SELECT 'vpn', 'ipv6_leak_protection', 'full_blocking', 'Full blocking', 'Vollständige Blockierung', 'positive', 10
  UNION ALL SELECT 'vpn', 'ipv6_leak_protection', 'tunnel_ipv6', 'Tunnel IPv6', 'IPv6-Tunnel', 'positive', 20
  UNION ALL SELECT 'vpn', 'ipv6_leak_protection', 'not_available', 'Not available', 'Nicht verfügbar', 'warning', 30
  ...
  UNION ALL SELECT 'vpn', 'webrtc_leak_prevention', 'not_available', 'Not available', 'Nicht verfügbar', 'tradeoff', 30
  ...
  UNION ALL SELECT 'vpn', 'data_cap', 'over_10gb', 'Over 10 GB', 'Über 10 GB', 'neutral', 20
  ...
  UNION ALL SELECT 'vpn', 'streaming_unblock_capability', 'reliable', 'Reliable', 'Zuverlässig', 'positive', 10
  UNION ALL SELECT 'vpn', 'streaming_unblock_capability', 'partial', 'Partial', 'Teilweise', 'neutral', 20
  UNION ALL SELECT 'vpn', 'streaming_unblock_capability', 'limited', 'Limited', 'Eingeschränkt', 'tradeoff', 30
  ...
  UNION ALL SELECT 'vpn', 'p2p_torrenting_allowed', 'restricted', 'Restricted', 'Eingeschränkt', 'tradeoff', 30
  ...
  UNION ALL SELECT 'vpn', 'split_tunneling', 'not_available', 'Not available', 'Nicht verfügbar', 'tradeoff', 40
  UNION ALL SELECT 'vpn', 'anonymous_signup', 'no_identity_required', 'No identity required', 'Keine Identität erforderlich', 'positive', 10
  ...
  UNION ALL SELECT 'vpn', 'anonymous_signup', 'full_identity', 'Full identity', 'Vollständige Identität', 'warning', 40
  UNION ALL SELECT 'vpn', 'client_source_model', 'open_source', 'Open source', 'Open Source', 'positive', 10
  UNION ALL SELECT 'vpn', 'client_source_model', 'source_available', 'Source available', 'Quellcode verfügbar', 'neutral', 20
  UNION ALL SELECT 'vpn', 'client_source_model', 'partial_open', 'Partial open', 'Teilweise offen', 'tradeoff', 30
  UNION ALL SELECT 'vpn', 'client_source_model', 'proprietary', 'Proprietary', 'Proprietär', 'warning', 40
  UNION ALL SELECT 'vpn', 'server_source_model', 'open_source', 'Open source', 'Open Source', 'positive', 10
  UNION ALL SELECT 'vpn', 'server_source_model', 'source_available', 'Source available', 'Quellcode verfügbar', 'neutral', 20
  UNION ALL SELECT 'vpn', 'server_source_model', 'partial_open', 'Partial open', 'Teilweise offen', 'tradeoff', 30
  UNION ALL SELECT 'vpn', 'server_source_model', 'proprietary', 'Proprietary', 'Proprietär', 'warning', 40
  UNION ALL SELECT 'vpn', 'governance_model', 'community_driven', 'Community driven', 'Community-getrieben', 'positive', 10
  UNION ALL SELECT 'vpn', 'governance_model', 'foundation_backed', 'Foundation backed', 'Stiftungsunterstützt', 'positive', 20
  UNION ALL SELECT 'vpn', 'governance_model', 'corporate_independent', 'Corporate independent', 'Unabhängiges Unternehmen', 'neutral', 30
  ...
  UNION ALL SELECT 'vpn', 'fit_profiles', 'business', 'Business', 'Geschäftlich', 'neutral', 40
