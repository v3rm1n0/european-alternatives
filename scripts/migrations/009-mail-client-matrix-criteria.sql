...
VALUES
  ('mail-client', 'client_model_platforms', 'Client Model & Platforms', 'Client-Modell & Plattformen', 'Client type, platform support, distribution, account handling, and offline use.', 'Client-Typ, Plattformunterstützung, Vertrieb, Kontenverwaltung und Offline-Nutzung.', 100),
  ('mail-client', 'accounts_protocols', 'Accounts & Protocols', 'Konten & Protokolle', 'Supported account types, mail protocols, authentication, identities, and setup.', 'Unterstützte Kontotypen, Mail-Protokolle, Authentifizierung, Identitäten und Einrichtung.', 200),
  ...
  ('mail-client', 'composing_productivity', 'Composing & Productivity', 'Schreiben & Produktivität', 'Compose formats, signatures, templates, scheduling, attachments, and shortcuts.', 'Schreibformate, Signaturen, Vorlagen, Zeitplanung, Anhänge und Tastaturkurzbefehle.', 400),
  ('mail-client', 'security_privacy', 'Security & Privacy', 'Sicherheit & Datenschutz', 'Remote content controls, tracker protection, warnings, credential storage, app lock, and network privacy.', 'Kontrollen für externe Inhalte, Tracking-Schutz, Warnungen, Zugangsdaten-Speicherung, App-Sperre und Netzwerkdatenschutz.', 500),
  ('mail-client', 'encryption_signing', 'Encryption & Signing', 'Verschlüsselung & Signaturen', 'OpenPGP, S/MIME, Autocrypt, key management, local encryption, and recipient policies.', 'OpenPGP, S/MIME, Autocrypt, Schlüsselverwaltung, lokale Verschlüsselung und Empfängerrichtlinien.', 600),
  ('mail-client', 'data_portability_sync', 'Data, Portability & Sync', 'Daten, Portabilität & Sync', 'Import, export, local storage, profile portability, contacts, and calendars.', 'Import, Export, lokaler Speicher, Profil-Portabilität, Kontakte und Kalender.', 700),
...
FROM (
  SELECT 'mail-client' AS category_id, 'client_model_platforms' AS group_key, 'client_model' AS criterion_key, 'Client model' AS label_en, 'Client-Modell' AS label_de, 'enum' AS value_type, 'informational' AS semantics, 'optional' AS filter_mode, 1010 AS sort_order, 'The main application model used to deliver the mail client experience.' AS help_text_en, 'Das grundlegende Client-Modell für die Mail-Client-Nutzung.' AS help_text_de
  UNION ALL SELECT 'mail-client', 'client_model_platforms', 'supported_platforms', 'Supported platforms', 'Unterstützte Plattformen', 'multi_enum', 'informational', 'multi_select', 1020, 'Operating systems and app platforms with maintained client support.', 'Betriebssysteme und App-Plattformen mit gepflegter Client-Unterstützung.'
  UNION ALL SELECT 'mail-client', 'client_model_platforms', 'distribution_channels', 'Distribution channels', 'Vertriebskanäle', 'multi_enum', 'informational', 'multi_select', 1030, 'Documented ways users can install or receive the client application.', 'Dokumentierte Wege, über die Nutzer die Client-App installieren oder beziehen können.'
  ...
  UNION ALL SELECT 'mail-client', 'accounts_protocols', 'supported_account_types', 'Supported account types', 'Unterstützte Kontotypen', 'multi_enum', 'informational', 'multi_select', 2010, 'Mail account families the client documents as supported.', 'Mail-Kontotypen, die der Client als unterstützt dokumentiert.'
  UNION ALL SELECT 'mail-client', 'accounts_protocols', 'mail_protocols', 'Mail protocols', 'Mail-Protokolle', 'multi_enum', 'beneficial', 'multi_select', 2020, 'Mail access and submission protocols supported by the client.', 'Vom Client unterstützte Protokolle für Mail-Zugriff und Versand.'
  UNION ALL SELECT 'mail-client', 'accounts_protocols', 'oauth2_support', 'OAuth2 account support', 'OAuth2-Kontounterstützung', 'multi_enum', 'beneficial', 'multi_select', 2030, 'OAuth2 provider flows or generic OAuth2 support available during account setup.', 'OAuth2-Anbieterabläufe oder generische OAuth2-Unterstützung bei der Kontoeinrichtung.'
  ...
  UNION ALL SELECT 'mail-client', 'accounts_protocols', 'identity_alias_management', 'Identity and alias management', 'Identitäts- und Aliasverwaltung', 'multi_enum', 'informational', 'multi_select', 2050, 'Controls for sender identities, signatures, aliases, and reply addresses.', 'Kontrollen für Absenderidentitäten, Signaturen, Aliasse und Antwortadressen.'
  UNION ALL SELECT 'mail-client', 'reading_organization', 'conversation_threading', 'Conversation threading', 'Konversationsansicht', 'boolean', 'beneficial', 'optional', 3010, 'Whether related messages can be grouped into conversation threads.', 'Ob zusammengehörige Nachrichten in Konversationsansichten gruppiert werden können.'
  UNION ALL SELECT 'mail-client', 'reading_organization', 'folder_label_model', 'Folder and label model', 'Ordner- und Label-Modell', 'enum', 'tradeoff', 'optional', 3020, 'How the client represents message organization through folders, labels, or virtual views.', 'Wie der Client Nachrichtenorganisation über Ordner, Labels oder virtuelle Ansichten abbildet.'
  UNION ALL SELECT 'mail-client', 'reading_organization', 'local_search_scope', 'Local search scope', 'Umfang der lokalen Suche', 'enum', 'informational', 'optional', 3030, 'What local search indexes can cover after mail is synchronized to the client.', 'Welche Inhalte lokale Suchindizes nach der Synchronisierung in den Client abdecken können.'
  ...
  UNION ALL SELECT 'mail-client', 'reading_organization', 'notification_controls', 'Notification controls', 'Benachrichtigungskontrollen', 'multi_enum', 'informational', 'multi_select', 3060, 'Controls for account, folder, schedule, or priority-based notifications.', 'Kontrollen für konto-, ordner-, zeitplan- oder prioritätsbasierte Benachrichtigungen.'
  UNION ALL SELECT 'mail-client', 'composing_productivity', 'compose_format_modes', 'Compose format modes', 'Schreibformat-Modi', 'multi_enum', 'informational', 'multi_select', 4010, 'Message composition formats and per-message compose controls.', 'Nachrichtenformate und Kontrollmöglichkeiten beim Schreiben einzelner Nachrichten.'
  ...
  UNION ALL SELECT 'mail-client', 'composing_productivity', 'scheduled_send_model', 'Scheduled send model', 'Modell für geplantes Senden', 'enum', 'tradeoff', 'optional', 4030, 'How the client supports composing now and sending later.', 'Wie der Client das Schreiben jetzt und Senden später unterstützt.'
  UNION ALL SELECT 'mail-client', 'composing_productivity', 'attachment_handling', 'Attachment handling', 'Anhangsverwaltung', 'multi_enum', 'beneficial', 'multi_select', 4040, 'Attachment workflows and warnings available while composing mail.', 'Anhangsabläufe und Warnungen, die beim Schreiben von Mail verfügbar sind.'
  UNION ALL SELECT 'mail-client', 'composing_productivity', 'keyboard_shortcuts', 'Keyboard shortcuts', 'Tastaturkurzbefehle', 'boolean', 'beneficial', 'optional', 4050, 'Whether documented keyboard shortcuts support common reading and composing tasks.', 'Ob dokumentierte Tastaturkurzbefehle häufige Lese- und Schreibaufgaben unterstützen.'
  UNION ALL SELECT 'mail-client', 'security_privacy', 'remote_content_blocking', 'Remote-content blocking', 'Blockieren externer Inhalte', 'boolean', 'beneficial', 'must_match', 5010, 'Whether remote images or external message content can be blocked by default.', 'Ob externe Bilder oder externe Nachrichteninhalte standardmäßig blockiert werden können.'
  ...
  UNION ALL SELECT 'mail-client', 'security_privacy', 'phishing_malware_warnings', 'Phishing and malware warnings', 'Phishing- und Malware-Warnungen', 'multi_enum', 'beneficial', 'multi_select', 5030, 'Warnings for suspicious links, spoofing, risky attachments, or scanner integrations.', 'Warnungen für verdächtige Links, Spoofing, riskante Anhänge oder Scanner-Integrationen.'
  ...
  UNION ALL SELECT 'mail-client', 'security_privacy', 'proxy_tor_support', 'Proxy or Tor support', 'Proxy- oder Tor-Unterstützung', 'multi_enum', 'tradeoff', 'multi_select', 5060, 'Network proxy or Tor-related options documented for mail connections.', 'Dokumentierte Proxy- oder Tor-bezogene Optionen für Mail-Verbindungen.'
  UNION ALL SELECT 'mail-client', 'encryption_signing', 'openpgp_support', 'OpenPGP support', 'OpenPGP-Unterstützung', 'enum', 'tradeoff', 'must_match', 6010, 'How OpenPGP encryption and signing are supported by the client.', 'Wie OpenPGP-Verschlüsselung und Signaturen durch den Client unterstützt werden.'
  UNION ALL SELECT 'mail-client', 'encryption_signing', 'smime_support', 'S/MIME support', 'S/MIME-Unterstützung', 'enum', 'tradeoff', 'optional', 6020, 'How S/MIME certificates and signing or encryption workflows are supported.', 'Wie S/MIME-Zertifikate sowie Signatur- oder Verschlüsselungsabläufe unterstützt werden.'
  UNION ALL SELECT 'mail-client', 'encryption_signing', 'autocrypt_support', 'Autocrypt support', 'Autocrypt-Unterstützung', 'boolean', 'beneficial', 'optional', 6030, 'Whether the client supports Autocrypt for key discovery and encrypted mail setup.', 'Ob der Client Autocrypt für Schlüsselerkennung und Einrichtung verschlüsselter Mail unterstützt.'
  UNION ALL SELECT 'mail-client', 'encryption_signing', 'key_management_model', 'Key management model', 'Modell zur Schlüsselverwaltung', 'enum', 'tradeoff', 'optional', 6040, 'Where encryption keys are managed for signing and encryption workflows.', 'Wo Schlüssel für Signatur- und Verschlüsselungsabläufe verwaltet werden.'
  UNION ALL SELECT 'mail-client', 'encryption_signing', 'encrypted_local_storage', 'Encrypted local storage', 'Verschlüsselter lokaler Speicher', 'boolean', 'beneficial', 'optional', 6050, 'Whether the local mail store or profile can be encrypted by the client.', 'Ob der lokale Mail-Speicher oder das Profil durch den Client verschlüsselt werden kann.'
  UNION ALL SELECT 'mail-client', 'encryption_signing', 'per_recipient_encryption_policy', 'Per-recipient encryption policy', 'Verschlüsselungsrichtlinie pro Empfänger', 'boolean', 'beneficial', 'optional', 6060, 'Whether encryption or signing defaults can be configured per recipient.', 'Ob Verschlüsselungs- oder Signaturvorgaben pro Empfänger konfiguriert werden können.'
  ...
  UNION ALL SELECT 'mail-client', 'data_portability_sync', 'local_storage_format', 'Local storage format', 'Lokales Speicherformat', 'enum', 'tradeoff', 'optional', 7030, 'The documented format used for local mail storage or synchronized cache data.', 'Das dokumentierte Format für lokale Mail-Speicherung oder synchronisierte Cache-Daten.'
  UNION ALL SELECT 'mail-client', 'data_portability_sync', 'profile_backup_portability', 'Profile backup and portability', 'Profil-Backup und Portabilität', 'enum', 'beneficial', 'optional', 7040, 'How client profiles and account settings can be backed up or moved.', 'Wie Client-Profile und Kontoeinstellungen gesichert oder verschoben werden können.'
  UNION ALL SELECT 'mail-client', 'data_portability_sync', 'contacts_sync_support', 'Contacts sync support', 'Kontakte-Sync-Unterstützung', 'multi_enum', 'beneficial', 'multi_select', 7050, 'Contact synchronization, import, or export paths supported by the client.', 'Vom Client unterstützte Wege für Kontakte-Sync, Import oder Export.'
  UNION ALL SELECT 'mail-client', 'data_portability_sync', 'calendar_sync_support', 'Calendar sync support', 'Kalender-Sync-Unterstützung', 'multi_enum', 'beneficial', 'multi_select', 7060, 'Calendar synchronization, import, export, or absence documented for the client.', 'Vom Client dokumentierte Kalender-Sync-, Import-, Export- oder Nichtverfügbarkeitsoptionen.'
  UNION ALL SELECT 'mail-client', 'customization_integrations', 'add_on_extension_support', 'Add-on or extension support', 'Add-on- oder Erweiterungsunterstützung', 'boolean', 'tradeoff', 'optional', 8010, 'Whether the client supports add-ons, plugins, or extensions.', 'Ob der Client Add-ons, Plugins oder Erweiterungen unterstützt.'
  UNION ALL SELECT 'mail-client', 'customization_integrations', 'theming_layout_options', 'Theming and layout options', 'Theme- und Layoutoptionen', 'multi_enum', 'informational', 'multi_select', 8020, 'Layout, density, theme, or visual customization options available to users.', 'Layout-, Dichte-, Theme- oder visuelle Anpassungsoptionen für Nutzer.'
  UNION ALL SELECT 'mail-client', 'customization_integrations', 'external_tool_integration', 'External tool integration', 'Integration externer Werkzeuge', 'multi_enum', 'tradeoff', 'multi_select', 8030, 'Integration points for editors, command hooks, calendars, address books, or password managers.', 'Integrationspunkte für Editoren, Befehls-Hooks, Kalender, Adressbücher oder Passwortmanager.'
  UNION ALL SELECT 'mail-client', 'customization_integrations', 'managed_deployment', 'Managed deployment', 'Verwaltete Bereitstellung', 'enum', 'informational', 'optional', 8040, 'Deployment or configuration management paths useful for teams or managed devices.', 'Bereitstellungs- oder Konfigurationsverwaltungswege für Teams oder verwaltete Geräte.'
  UNION ALL SELECT 'mail-client', 'accessibility_fit', 'accessibility_features', 'Accessibility features', 'Barrierefreiheitsfunktionen', 'multi_enum', 'beneficial', 'multi_select', 9010, 'Accessibility affordances documented for keyboard, screen reader, contrast, type, or motion needs.', 'Dokumentierte Barrierefreiheitsfunktionen für Tastatur, Screenreader, Kontrast, Schrift oder Bewegung.'
  UNION ALL SELECT 'mail-client', 'accessibility_fit', 'localization_scope', 'Localization scope', 'Lokalisierungsumfang', 'enum', 'informational', 'optional', 9020, 'How broadly the client interface is localized for non-English users.', 'Wie breit die Client-Oberfläche für nicht-englische Nutzer lokalisiert ist.'
  UNION ALL SELECT 'mail-client', 'accessibility_fit', 'fit_profiles', 'Fit profiles', 'Eignungsprofile', 'multi_enum', 'informational', 'multi_select', 9030, 'Common usage profiles the client is especially suited to support.', 'Typische Nutzungsprofile, für die der Client besonders geeignet ist.'
) AS d
...
FROM (
  ...
  UNION ALL SELECT 'mail-client', 'client_model', 'cross_platform_app', 'Cross-platform app', 'Plattformübergreifende App', 'neutral', 30
  ...
  UNION ALL SELECT 'mail-client', 'identity_alias_management', 'multiple_identities', 'Multiple identities', 'Mehrere Identitäten', 'positive', 10
  ...
  UNION ALL SELECT 'mail-client', 'local_search_scope', 'attachments', 'Attachments', 'Anhänge', 'positive', 30
  UNION ALL SELECT 'mail-client', 'local_search_scope', 'encrypted_index', 'Encrypted index', 'Verschlüsselter Index', 'tradeoff', 40
  ...
  UNION ALL SELECT 'mail-client', 'notification_controls', 'sender_priority', 'Sender priority', 'Absenderpriorität', 'positive', 40
  ...
  UNION ALL SELECT 'mail-client', 'scheduled_send_model', 'server_assisted', 'Server-assisted', 'Serverseitig unterstützt', 'positive', 40
  ...
  UNION ALL SELECT 'mail-client', 'attachment_handling', 'large_file_warning', 'Large-file warning', 'Warnung bei großen Dateien', 'positive', 30
  ...
  UNION ALL SELECT 'mail-client', 'attachment_handling', 'inline_image_controls', 'Inline image controls', 'Kontrollen für eingebettete Bilder', 'neutral', 50
  ...
  UNION ALL SELECT 'mail-client', 'phishing_malware_warnings', 'suspicious_link_warning', 'Suspicious link warning', 'Warnung bei verdächtigen Links', 'positive', 10
  ...
  UNION ALL SELECT 'mail-client', 'credential_storage_model', 'system_keychain', 'System keychain', 'System-Schlüsselbund', 'positive', 10
  UNION ALL SELECT 'mail-client', 'credential_storage_model', 'app_encrypted_store', 'App-encrypted store', 'App-verschlüsselter Speicher', 'neutral', 20
  ...
  UNION ALL SELECT 'mail-client', 'openpgp_support', 'key_import_export', 'Key import/export', 'Schlüsselimport/-export', 'positive', 40
  ...
  UNION ALL SELECT 'mail-client', 'key_management_model', 'external_key_manager', 'External key manager', 'Externe Schlüsselverwaltung', 'tradeoff', 10
  UNION ALL SELECT 'mail-client', 'key_management_model', 'integrated_key_manager', 'Integrated key manager', 'Integrierte Schlüsselverwaltung', 'positive', 20
  UNION ALL SELECT 'mail-client', 'key_management_model', 'system_keychain', 'System keychain', 'System-Schlüsselbund', 'positive', 30
  UNION ALL SELECT 'mail-client', 'key_management_model', 'hardware_token', 'Hardware token', 'Hardware-Token', 'positive', 40
  UNION ALL SELECT 'mail-client', 'key_management_model', 'manual_files', 'Manual key files', 'Manuelle Schlüsseldateien', 'tradeoff', 50
  ...
  UNION ALL SELECT 'mail-client', 'local_storage_format', 'proprietary_database', 'Proprietary database', 'Proprietäre Datenbank', 'warning', 40
  ...
  UNION ALL SELECT 'mail-client', 'theming_layout_options', 'message_pane_layouts', 'Message pane layouts', 'Layouts für Nachrichtenbereich', 'neutral', 30
  ...
  UNION ALL SELECT 'mail-client', 'localization_scope', 'major_languages', 'Major languages', 'Große Sprachen', 'neutral', 20
  UNION ALL SELECT 'mail-client', 'localization_scope', 'community_translations', 'Community translations', 'Community-Übersetzungen', 'positive', 30
